...
macro(setup_compiler)
  ...
  if(${_num_architectures_compiler} GREATER 1)
    message(
      SEND_ERROR
        "Too many architectures configured: ${_list_architectures_compiler}")
  endif()
  ...
  # warnings
  set(CMAKE_CXX_FLAGS
      "${CMAKE_CXX_FLAGS} -Wall -Wextra -Wconversion -Wpedantic")
  ...
  if(FETCH_ENABLE_COVERAGE)
    if(_is_clang_compiler)
      set(CMAKE_CXX_FLAGS_DEBUG
          "-g -fprofile-instr-generate -fcoverage-mapping -O0")
    else()
      message(
        SEND_ERROR
          "Coverage flag enabled but clang compiler not found, CMake will exit."
        )
    endif()
  ...
  if(${_debug_sanitizer_parameter_length} GREATER 0)
    if(_is_clang_compiler)
      ...
      message(
        SEND_ERROR
          "Debug sanitizers are only available for clang based compilers")
    endif()
  endif()
...
function(configure_vendor_targets)
  ...
  if(OPENSSL_USE_STATIC_LIBS)
    target_link_libraries(vendor-openssl
                          INTERFACE ${CMAKE_DL_LIBS} ${CMAKE_THREAD_LIBS_INIT})
  ...
  target_include_directories(
    vendor-asio
    INTERFACE ${FETCH_ROOT_VENDOR_DIR}/asio/asio/include)
  ...
  # required for latest version of Xcode: Apple LLVM version 10.0.1
  # (clang-1001.0.46.3). In this version the string view has been removed from:
  # <experimental/string_view>. This will need to be set across all compilers
  # when C++17 support is standard.
  if(APPLE)
  ...
  if(FETCH_ENABLE_BENCHMARKS)
    set(BENCHMARK_ENABLE_TESTING
        OFF
        CACHE BOOL "Suppress google benchmark default tests" FORCE)
    add_subdirectory(${FETCH_ROOT_VENDOR_DIR}/benchmark)
  endif(FETCH_ENABLE_BENCHMARKS)

  # mio vendor library
  add_library(vendor-mio INTERFACE)
  target_include_directories(vendor-mio
                             INTERFACE ${FETCH_ROOT_VENDOR_DIR}/mio/include)

  # MsgPack
  add_library(vendor-msgpack INTERFACE)
  target_include_directories(vendor-msgpack
                             INTERFACE ${FETCH_ROOT_VENDOR_DIR}/msgpack/include)
    ...
    elseif(FETCH_IS_JETBRAINS_IDE)
      ...
      if(FETCH_WARNINGS_AS_ERRORS)
        set(CMAKE_CXX_CLANG_TIDY "${CLANG_TIDY_EXE}"
                                 "-checks=file -warnings-as-errors=*")
      else()
