...
#            print
            ret.append( {"arguments": [q.type.spelling for q in x.get_arguments()]} )
    return ret

def GetMethods(n, classname, known_types = None):
    if known_types == None: known_types = {}
    ret = {}
    operators = ["=", "==", "!=", "<", ">", "<=", ">=", "+", "-", "/", "*", "+=", "-=", "/=", "*=", "&", "|", "&=", "|="]                    
    for x in n.get_children():
        kind = str(x.kind).split(".")[1]
        ...
        if kind == "CXX_METHOD":
            ...
            if x.is_pure_virtual_method():
                continue
            if x.is_static_method(): continue
            
            opname = x.spelling.replace("operator", "")
            is_operator = opname in operators
            opargs = ["py::self", "py::self"]
            updateType = lambda x: x if not x in known_types else known_types[x]
            arguments = [updateType(q.type.spelling) for q in x.get_arguments()]
            ...
            if is_operator:
                ...
                if a.startswith("const "): a = a.split("const ", 1)[1]
                if a.endswith("&"): a = a.rsplit("&", 1)[0]
                a = a.replace(" ", "").strip()                
                c = classname.replace(" ", "")
            ...
            fullsign = rettype+" ("+classname+"::*)("+", ".join(arguments) +")"
...
def GetTemplateClassParameters(n):
    ...
    for x in n.get_children():
        ...
        if kind == "TEMPLATE_TYPE_PARAMETER":
            ret.append({"name": x.spelling, "definition": "typename " + x.spelling})
        if kind == "TEMPLATE_NON_TYPE_PARAMETER":            
            ret.append({"name": x.spelling, "definition": x.type.spelling + " " + x.spelling }) 
    return ret    
...
def Traverse(n, filename, namespace = None, export = None):
    ...
    if not namestr in export: export[namestr] = { "namespace": copy.copy(namespace), "classes": []}
...
def CreatePythonFile(filename, outputdir, translation_unit):
    ...
    shortinc = filename.rsplit("include/", 1)[1] if "include/" in filename else filename
    guard = shortinc 
    ...
    with open(output, "w") as fb:
        fb.write(t.render(includes= includes, HEADER_GUARD = guard, exporting = data, len = len))
    ...
    for ns, d in data.iteritems():
        for cls in d["classes"]:
            ...
            build_functions.append(  {"namespace": ns, "function": "Build" + classname} )
...
if __name__ == "__main__":
    ...
    for root, dir, files in os.walk(sys.argv[1]):
        ...
        for f in files:
            if f.endswith(".hpp"):
                ...
                translation_unit = index.parse(filename, ['-x', 'c++', '-std=c++11', "-I"+sys.argv[1]] + addincl)
    ...
    createdns = {}
    def create_ns(ns):
        if len(ns) ==  0: return "module"
        cpp_name = "::".join(ns)
        if cpp_name in createdns: return createdns[cpp_name]
        
        nns = "_".join(ns)
        prev = create_ns( ns[:-1] )

        code = "py::module ns_%s = %s.def_submodule(\"%s\")" %(nns, prev, ns[-1])
    ...
    def get_or_create_ns(ns):
        if ns in createdns: return createdns[ns]
        parts = ns.split("::")
