...
save_figures = True
n_reps = 100
max_lens = {'Array': 6000, 'Tensor': 100000}
imgformat = 'png'
...
def linear_fit(bms, bm_type, max_len=None):
...
for (bm_type, param_bm) in param_bms.items():
    ...
    if benchmarks[ind0]['class'] in max_lens:
        param_bm['lfit'] = linear_fit(
            benchmarks, bm_type, max_lens[benchmarks[ind0]['class']])
    else:
        param_bm['lfit'] = linear_fit(benchmarks, bm_type)
...
def least_squares_opcode_times(benchmarks, optimes={}, prim_type=None):

    basic_bms = [bm for (ind, bm) in benchmarks.items()
                 if bm['class'] == 'Basic' or prim_type in bm['name']]

    basic_ops = set()
    for bm in basic_bms:
        basic_ops = basic_ops.union(set(bm['opcodes']))

    known_ops = set(optimes.keys())
    new_ops = basic_ops - known_ops

    new_ops = np.array(list(new_ops))
    op2ind = {op: ind for (ind, op) in enumerate(basic_ops)}

    A = np.zeros((n_reps*len(basic_bms), len(basic_ops)))
...
#    for (i, op) in enumerate(basic_ops):
    ...
    optimes = {op: x[i] for (i, op) in enumerate(basic_ops)}

    print(np.vstack((basic_ops, x)).T)
...
optimes, A, b = least_squares_opcode_times(benchmarks, 'Basic')
