...
param_bms = {type_name: {'inds': [ind for (ind, bm) in benchmarks.items() if bm['type'] == type_name],
                         'lfit': linear_fit(benchmarks, type_name, max_len),
...
def least_squares_opcode_times(benchmarks, prim_type=None):
    ...
    basic_ops = np.array(list(basic_ops))
    ...
    optimes = np.linalg.lstsq(A, b, rcond=None)[0]

    print(np.vstack((basic_ops, optimes)).T)
...
optimes = least_squares_opcode_times(benchmarks, 'Int8')
