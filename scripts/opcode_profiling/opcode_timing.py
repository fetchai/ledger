...
bm_filter = 'Basic|Object|Primitive|Math|Array|Tensor|Crypto'
...
# Primtive types used in primitive operation benchmarks
op_prims = {'Int8', 'Int16', 'Int32', 'Int64', 'UInt8', 'UInt16', 'UInt32', 'UInt64',
            'Float32', 'Float64', 'Fixed32', 'Fixed64', 'Fixed128'}

# Primitive types used in math function benchmarks
...
# Delete break and continue benchmarks for purposes of fitting (negative cpu times)
benchmarks_fit = benchmarks.copy()
for ind in benchmarks.keys():
    if benchmarks[ind]['name'] in {'Break', 'Continue'}:
        del benchmarks_fit[ind]

# Solve linear least-squares problem to estimate isolated opcode times
optimes = {'Base': dict()}
vm_least_squares.opcode_times(benchmarks_fit, optimes['Base'], {'Basic'})

# For primitive-based opcodes, use already computed base opcode times
for prim in op_prims:
    optimes[prim] = optimes['Base'].copy()
    vm_least_squares.opcode_times(benchmarks_fit, optimes[prim], {
                                  'Prim'}, prim_type=prim)

# Compute math function opcode times for relevant primitives
for prim in math_prims:
    vm_least_squares.opcode_times(benchmarks_fit, optimes[prim], {
                                  'Math'}, prim_type=prim)

# Update benchmark data with isolated opcode times
...
# Delete the redundant base opcodes for reporting purposes
for prim in op_prims:
...
# Define parameterized benchmarks whose fit slope satisfies threshold
