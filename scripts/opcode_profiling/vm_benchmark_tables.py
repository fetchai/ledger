...
def linear_fit_table(param_bms, n_reps, bm_cls):
    ...
    for (name, bm) in param_bms.items():
        if bm_cls in name:
            lfit_table.append([name, bm['lfit'][0], bm['lfit'][1], bm['agg_net_mean'],  # bm['agg_net_median'],
                               # bm['opcodes'], bm['baseline'],
                               bm['agg_net_stderr'],
                               bm['net_opcodes']])

    lfit_table = sorted(lfit_table, key=lambda l: l[1], reverse=True)

    headers = ['Benchmark (' + str(n_reps) + ' reps)', 'Slope (ns/char)',
               'Intercept (ns)', 'Mean (ns)',  # 'Median (ns)',
               'Std error (ns)',  # 'Opcodes', 'Baseline',
               'Net opcodes']
