...
def primitive_table(benchmarks, n_reps, bm_class):
    ...
    prim_bms = {ind : bm for (ind,bm) in benchmarks.items() if bm['class'] == bm_class }
    prim_types = list({bm['name'].split('_')[1] for bm in prim_bms.values()} - {''})
    ...
    for bm_type in bm_types: 
        ...
        for prim in prim_types:
            tab_bms = [bm for bm in prim_bms.values() if [bm_type,prim] == bm['name'].split('_')]
            if len(tab_bms) == 1:
                tab_bm = tab_bms[0]
                type_row.append("{:.2f} \u00B1 {:.2f}".format(tab_bm['net_mean'], tab_bm['net_stderr']))
            else:
                type_row.append('--')
    ...
    headers_int = ['Benchmark (' + str(n_reps) + ' reps)'] + [h + ' (ns)' for h in int_prim_types]
    table_int = [[row[0]] + [elem for (i,elem) in enumerate(row[1:]) if prim_types[i] in int_prim_types] for row in prim_bm_table]
    
    headers_fp = ['Benchmark (' + str(n_reps) + ' reps)'] + [h + ' (ns)' for h in fp_prim_types]
    table_fp = [[row[0]] + [elem for (i,elem) in enumerate(row[1:]) if prim_types[i] in fp_prim_types] for row in prim_bm_table]
