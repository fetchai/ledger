...
baselines = {oprow[1]: oprow[2] for oprow in oprows}
opcodes = {oprow[1]: [int(opcode) for (i, opcode) in enumerate(oprow[3:])] for oprow in oprows}
...
for bm in bm_inds.values():
    ...
    net_opcodes = [x for x in bm_opcodes if x not in base_opcodes or base_opcodes.remove(x)]

    bm_opcodes_copy = bm_opcodes.copy()
    extra_opcodes = [x for x in base_opcodes if x not in bm_opcodes_copy or bm_opcodes_copy.remove(x)]

    table.append([bm_names[bm], bm_mean, bm_stderr, bm_opcodes, bm_names[base_bm], net_opcodes])

print('\n')
print(tabulate(table, headers=['Benchmark (' + str(num_reps) + ' reps)', 'Mean (ns)', 'Std error (ns)', 'Opcodes',
