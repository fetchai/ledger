...
void MtCore::startListeners(IKarmaPolicy *karmaPolicy)
{
  ...
  if (!config_.ws_uri().empty())
  {
    ...
    auto task_ws = std::make_shared<OefListenerStarterTask<EndpointWebSocket>>(ws_uri.port,
    listeners, core, initialFactoryCreator, karmaPolicy, endpointConfigWS); task_ws -> submit();*/
