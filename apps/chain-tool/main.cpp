...
int main(int argc, char **argv)
{
  ...
  std::string revert_to_block_hash = "";
  ...
  parser.add(create_trimmed_tx_store, "trim-tx-db",
             ...
             false);
  parser.add(revert_to_block_hash, "revert-to-block-hash",
             "Change the main chain's saved HEAD (must use hex string beginning 0x)",
             std::string{""});
  parser.Parse(argc, argv);

  if (revert_to_block_hash != "")
  {
    std::cout << "Loading up old main chain." << std::endl;

    MainChain chain{false, ledger::MainChain::Mode::LOAD_PERSISTENT_DB};

    fetch::byte_array::ConstByteArray hash_from_hex = revert_to_block_hash;
    hash_from_hex                                   = hash_from_hex.SubArray(2);
    std::cout << "Attempting revert to block hash: " << hash_from_hex << std::endl;
    hash_from_hex = hash_from_hex.FromHex();

    auto head_to_set = chain.GetBlock(hash_from_hex);

    if (!head_to_set)
    {
      std::cout << "Didn't find the block we want to set as head. Exiting." << std::endl;
      exit(1);
    }
    else
    {
      std::cout << "Found block we want to set as head! Attempting to revert. Block num: "
                << head_to_set->block_number << "\n";
      chain.SetHeadHash(hash_from_hex);
    }
