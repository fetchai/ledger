...
int main(int argc, char **argv)
{
  ...
  parser.add(revert_to_block_hash, "revert-to-block-hash",
             "Change the main chain's saved HEAD",
             std::string{""});
  ...
  if(revert_to_block_hash != "")
  {
    ...
    {
      std::cout << "Found block we want to set as head! Attempting to revert. Block num: " << head_to_set->block_number << "\n";
  ...
  {
    std::cout << "Failed to parse parameter for MC block head hex edit." << std::endl;
  }
