...
Constellation::Config BuildConstellationConfig(std::string const &genesis_file_loc,
                                               uint16_t           start_port)
{
...
protected:
  ...
  void StartNodes(uint32_t nodes, uint32_t of_which_are_miners)
  {
    ...
    {
      constellations_.emplace_back(std::make_unique<fetch::constellation::Constellation>(
          certificates_[i],
          BuildConstellationConfig(genesis_file_location_, uint16_t(8000 + (i * 100)))));
    ...
    for (std::size_t i = 0; i < nodes; ++i)
    {
      ...
      auto cb = [&constellations, i, random_milliseconds]() {
        ...
        {
          byte_array::ConstByteArray peer{"tcp://127.0.0.1:" +
                                          std::to_string(uint16_t(8001 + (j * 100)))};
