...
#include "base/src/cpp/proto_comms/ProtoMessageSender.hpp"
...
ProtoMessageEndpoint<TXType, Reader, Sender>::ProtoMessageEndpoint(std::shared_ptr<EndpointType> endpoint)
  : EndpointPipe<EndpointBase<TXType>>(std::move(endpoint))
{
}

template <typename TXType, typename Reader, typename Sender>
void ProtoMessageEndpoint<TXType, Reader, Sender>::setup(std::shared_ptr<ProtoMessageEndpoint>& myself)
{
...
template class ProtoMessageEndpoint<std::pair<Uri, std::shared_ptr<google::protobuf::Message>>, ProtoPathMessageReader, ProtoPathMessageSender>;
