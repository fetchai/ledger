...
struct Array : public Object
{
	...
	Array(const TypeId type_id, VM* vm, const size_t size):
		Object(type_id, vm), elements(size, T(0)) {}
	template <typename U,
...
private:
	...
	void HandleEqualityOp(const TypeId type_id, Value& lhsv, Value& rhsv)
	{
		switch (type_id)
		{
			...
			default: {
				Op::Apply(lhsv, rhsv, lhsv.variant.object, rhsv.variant.object);
				break;
			}
		}
	}
	...
	void HandleRelationalOp(const TypeId type_id, Value& lhsv, Value& rhsv)
	{
		switch (type_id)
		{
			...
			//case TypeId::String: {
				//Op::Apply(lhsv, rhsv, lhsv.variant.d, rhsv.variant.d);
				//break;
			//}
			default: {
				break;
	...
	void HandleIndexedArithmeticAssignmentOp(const TypeId type_id)
	{
		switch (type_id)
		{
			...
			case TypeId::Array_Matrix_Float32: {
				HandleObjectArrayIndexedArithmeticAssignmentOp<Op, MatrixFloat32*, Object*, MatrixFloat32*>();
				break;
			}
			case TypeId::Array_Matrix_Float64: {
				HandleObjectArrayIndexedArithmeticAssignmentOp<Op, MatrixFloat64*, Object*, MatrixFloat64*>();
				break;
			}
