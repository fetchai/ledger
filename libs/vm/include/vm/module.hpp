...
public:
  ...
  void CreateFreeFunction(std::string const & name, ReturnType (*f)(VM *, Ts...))
  {
    ...
    auto        compiler_setup_function = [name, opcode, type_id_array, return_type_id]
                                          (Compiler *compiler)
    {
    ...
    OpcodeHandler handler = [f](VM *vm)
    {
      InvokeFreeFunction(vm, vm->instruction_->type_id, f);
    };
  ...
  ClassInterface<ObjectType> CreateClassType(std::string const & name)
  {
    ...
    auto compiler_setup_function = [name, type_id]
                                   (Compiler *compiler)
    {
  ...
  TypeId CreateTemplateInstantiationType(TypeId template_type_id)
  {
    ...
    TypeIdArray type_id_array           = GetTypeIds(type_index_array);
    auto        compiler_setup_function = [type_id, template_type_id, type_id_array]
                                          (Compiler *compiler)
    {
