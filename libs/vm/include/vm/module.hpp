...
public:
  ...
  Module(Opcode const &first_opcode = Opcode::StartOfUserOpcodes, TypeId first_type_id = TypeId::StartOfUserTypes ) :
  ...
  Module& ExportFunction(std::string name, R (*function)(Args...) )
  {
    ...
    AddOpcode(opcode, [name, function](VM *vm) {
      ...
      using function_invoker = typename details::StaticOrFreeFunctionMagic<function_pointer, return_type, result_position>
                                                ::template LoopOver< last_argument_position,  Args...>;
...
public:
  ...
  : module_(module), type_( module.next_type_id() ) 
  {  }
  ...
  ClassInterface& Export(std::string name, R (T::*function)(Args...) )
  {
    ...
    module_.AddOpcode(opcode, [name, function](VM *vm) {
      ...
      using class_type = T;
      // TODO( tfr ): ask Robert if void results are populated on the stack.
      constexpr int result_position = int(sizeof...(Args)) + 1 - details::HasResult< R >::value; // +1 is to override this
      constexpr int last_argument_position = int(sizeof...(Args)) - 1;
      constexpr int this_position = int(sizeof...(Args));  

      // resetter that resets all elements until (but not including) the result position
      using stack_resetter = typename details::Resetter< result_position >;
      using function_invoker = typename details::MemberFunctionMagic<class_type, member_function_pointer, return_type, result_position>
                                      ::template LoopOver< last_argument_position, Args...>;
  ...
  ClassInterface& Constructor()
  {
    ...
    module_.AddSetupFunction([self, this, opcode, prototype](Analyser *a){
      ...
      a->CreateOpcodeTypeFunction(type_pointer_, Analyser::CONSTRUCTOR, args, type_pointer_, opcode);
    });
    ...
    module_.AddOpcode(opcode, [](VM *vm) {
      ...
      T TObj = details::ConstructorMagic<T>::template LoopOver<int(sizeof...(Args)) - 1, Args ...  >::Build(vm) ;
