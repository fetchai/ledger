...
Analyser::Analyser()
{
	...
	template_instantiation_type_ = CreateType("TemplateInstantiation",
		Type::Category::Primitive, TypeId::TemplateInstantiation);
	template_parameter_type1_ = CreateType("TemplateParameter1",
		Type::Category::Primitive, TypeId::TemplateParameter1);
	template_parameter_type2_ = CreateType("TemplateParameter2",
		Type::Category::Primitive, TypeId::TemplateParameter2);
	numeric_type_ = CreateType("Numeric",
		Type::Category::Primitive, TypeId::Numeric);
	...
	array_int8_type_ = CreateTemplateInstantiationType("Array<Int8>",
		TypeId::Array_Int8, array_template_type_, {int8_type_});
	array_byte_type_ = CreateTemplateInstantiationType("Array<Byte>",
		TypeId::Array_Byte, array_template_type_, {byte_type_});
	array_int16_type_ = CreateTemplateInstantiationType("Array<Int16>",
		TypeId::Array_Int16, array_template_type_, {int16_type_});
	array_uint16_type_ = CreateTemplateInstantiationType("Array<UInt16>",
		TypeId::Array_UInt16, array_template_type_, {uint16_type_});
	array_int32_type_ = CreateTemplateInstantiationType("Array<Int32>",
		TypeId::Array_Int32, array_template_type_, {int32_type_});
	array_uint32_type_ = CreateTemplateInstantiationType("Array<UInt32>",
		TypeId::Array_UInt32, array_template_type_, {uint32_type_});
	array_int64_type_ = CreateTemplateInstantiationType("Array<Int64>",
		TypeId::Array_Int64, array_template_type_, {int64_type_});
	array_uint64_type_ = CreateTemplateInstantiationType("Array<UInt64>",
		TypeId::Array_UInt64, array_template_type_, {uint64_type_});
	array_float32_type_ = CreateTemplateInstantiationType("Array<Float32>",
	...
	CreateOpcodeFunction("toInt8",
		{numeric_type_}, int8_type_, Opcode::ToInt8);
	CreateOpcodeFunction("toByte",
		{numeric_type_}, byte_type_, Opcode::ToByte);
	CreateOpcodeFunction("toInt16",
		{numeric_type_}, int16_type_, Opcode::ToInt16);
	CreateOpcodeFunction("toUInt16",
		{numeric_type_}, uint16_type_, Opcode::ToUInt16);
	CreateOpcodeFunction("toInt32",
		{numeric_type_}, int32_type_, Opcode::ToInt32);
	CreateOpcodeFunction("toUInt32",
		{numeric_type_}, uint32_type_, Opcode::ToUInt32);
	CreateOpcodeFunction("toInt64",
		{numeric_type_}, int64_type_, Opcode::ToInt64);
	CreateOpcodeFunction("toUInt64",
		{numeric_type_}, uint64_type_, Opcode::ToUInt64);
	CreateOpcodeFunction("toFloat32",
...
void Analyser::AddError(const Token& token, const std::string& message)
{
	...
	stream << "line " << token.line << ": " << "error: " << message;
	...
	const BlockNodePtr& function_definition_node)
{
	...
	for (int i=0; i<num_parameters; ++i)
	{
		...
		{
			std::stringstream stream;
			AddError(parameter_node->token,
				"parameter name '" + parameter_name + "' is already defined");
			++problems;
			continue;
		}
		...
		{
			AddError(parameter_type_node->token, "unknown type '" +
				parameter_type_node->token.text + "'");
			++problems;
			continue;
		}
		VariablePtr parameter_variable =
			CreateVariable(parameter_name, Variable::Category::Parameter);
	...
	if (return_type_supplied)
	{
		...
		{
			AddError(return_type_node->token, "unknown type '" +
				return_type_node->token.text + "'");
			++problems;
		}
	}
	else
	...
	FunctionPtr function = CreateUserFunction(name, parameter_types,
...
{
	ExpressionNodePtr identifier_node =
		ConvertToExpressionNodePtr(for_statement_node->children[0]);
	const std::string& name = identifier_node->token.text;
	...
	const NodePtr& var_statement_node)
{
	...
	{
		ExpressionNodePtr type_node =
			ConvertToExpressionNodePtr(var_statement_node->children[1]);
	...
	else if (var_statement_node->kind == Node::Kind::VarDeclarationTypedAssignmentStatement)
	{
		...
		ExpressionNodePtr expression_node =
			ConvertToExpressionNodePtr(var_statement_node->children[2]);
	...
	else
	{
		...
		if (AnnotateExpression(expression_node) == false)
			return;
		if ((expression_node->type->id == TypeId::Void) ||
			(expression_node->type->id == TypeId::Null))
		{
...
{
	ExpressionNodePtr expression_node =
		ConvertToExpressionNodePtr(conditional_node->children[0]);
...
TypePtr Analyser::IsAddSubtractCompatible
	(const ExpressionNodePtr& node, const ExpressionNodePtr& lhs, const ExpressionNodePtr& rhs)
{
...
TypePtr Analyser::IsMultiplyCompatible
	(const ExpressionNodePtr& node, const ExpressionNodePtr& lhs, const ExpressionNodePtr& rhs)
{
	const bool lhs_numeric = IsNumericType(lhs->type->id);
	const bool rhs_numeric = IsNumericType(rhs->type->id);
...
TypePtr Analyser::IsDivideCompatible
	(const ExpressionNodePtr& node, const ExpressionNodePtr& lhs, const ExpressionNodePtr& rhs)
{
	const bool lhs_numeric = IsNumericType(lhs->type->id);
	const bool rhs_numeric = IsNumericType(rhs->type->id);
...
bool Analyser::AnnotateExpression(const ExpressionNodePtr& node)
{
	switch (node->kind)
	{
		...
		case Node::Kind::Template:
		{
			...
			{
				AddError(node->token, "variable '" + node->token.text +
					"' has unresolved type");
				return false;
			}
...
bool Analyser::AnnotateUnaryMinusOp(const ExpressionNodePtr& node)
{
	...
	if (AnnotateExpression(operand) == false)
		return false;
	if ((IsNumericType(operand->type->id) == false) &&
		(IsMatrixType(operand->type->id) == false))
	{
...
bool Analyser::AnnotateRelationalOp(const ExpressionNodePtr& node)
{
	...
	ExpressionNodePtr lhs = ConvertToExpressionNodePtr(node->children[0]);
	ExpressionNodePtr rhs = ConvertToExpressionNodePtr(node->children[1]);
	if ((IsRelationalType(lhs->type->id) == false) ||
		(IsRelationalType(rhs->type->id) == false))
	{
		AddError(node->token, "incompatible types");
		return false;
	}
...
bool Analyser::AnnotateBinaryLogicalOp(const ExpressionNodePtr& node)
{
	...
	ExpressionNodePtr lhs = ConvertToExpressionNodePtr(node->children[0]);
	ExpressionNodePtr rhs = ConvertToExpressionNodePtr(node->children[1]);
	if ((lhs->type->id != TypeId::Bool) ||
		(rhs->type->id != TypeId::Bool))
	{
...
{
	ExpressionNodePtr lhs = ConvertToExpressionNodePtr(node->children[0]);
	if ((lhs->kind == Node::Kind::Identifier) ||
		(lhs->kind == Node::Kind::Template))
	{
...
bool Analyser::AnnotateDotOp(const ExpressionNodePtr& node)
{
	...
	const bool lhs_is_instance =
		(lhs->category == ExpressionNode::Category::Variable) ||
		(lhs->category == ExpressionNode::Category::LV) ||
		(lhs->category == ExpressionNode::Category::RV);

	if (lhs->type->IsPrimitiveType())
	{
		AddError(lhs->token, "primitive type '" + lhs->type->name +
			"' does not support member-access operator");
		return false;
	}
	SymbolPtr symbol;
	...
	if (symbol == nullptr)
	{
		AddError(lhs->token, "type '" + lhs->type->name +
...
bool Analyser::AnnotateInvokeOp(const ExpressionNodePtr& node)
{
	...
	if (lhs->category == ExpressionNode::Category::Function)
	{
		...
		std::vector<TypePtr> output_parameter_types;
		FunctionPtr f = FindMatchingFunction(lhs->fg, lhs->type, supplied_parameter_types,
			output_parameter_types);
		if (f == nullptr)
		{
			// No matching function, or ambiguous
			AddError(lhs->token, "unable to find matching function for '" +
				lhs->fg->name + "'");
			return false;
		}
		...
		if (f->kind == Function::Kind::OpcodeTypeFunction)
		{
			...
			{
				AddError(lhs->token, "function '" + lhs->fg->name +
					"' is a type function, not an instance function");
				return false;
			}
		}
		else if (f->kind == Function::Kind::OpcodeInstanceFunction)
		{
			...
			{
				AddError(lhs->token, "function '" + lhs->fg->name +
					"' is an instance function, not a type function");
				return false;
			}
		}
	...
	else if (lhs->category == ExpressionNode::Category::Type)
	{
		...
		if (symbol == nullptr)
		{
			AddError(lhs->token, "unable to find matching constructor for type '" +
				lhs->type->name + "'");
			return false;
		}
		FunctionGroupPtr fg = ConvertToFunctionGroupPtr(symbol);
		std::vector<TypePtr> output_parameter_types;
		FunctionPtr f = FindMatchingFunction(fg, lhs->type, supplied_parameter_types,
			output_parameter_types);
		if (f == nullptr)
		{
	...
	if (node->kind == Node::Kind::Template)
	{
		...
		else if (name == "Array")
		{
			...
			else
			{
				...
				TypePtr type = CreateTemplateInstantiationType(template_name,
					TypeId::Array, array_template_type_, {subtype});
				return type;
			}
		}
		else
...
TypePtr Analyser::CreateTemplateInstantiationType(const std::string& name, const TypeId id,
	const TypePtr& template_type, const std::vector<TypePtr>& template_parameter_types)
{
...
FunctionPtr Analyser::CreateOpcodeFunction(const std::string& name,
...
void Analyser::CreateOpcodeFunction(const std::string& name,
		...
		const Opcode opcode)
{
	AddFunction(symbols_, CreateOpcodeFunction(name,
		Function::Kind::OpcodeFunction, parameter_types, return_type, opcode));
}


void Analyser::CreateOpcodeTypeFunction(const TypePtr& type,
		const std::string& name,
		const std::vector<TypePtr>& parameter_types,
		const TypePtr& return_type,
		const Opcode opcode)
{
	AddFunction(type->symbols, CreateOpcodeFunction(name,
		Function::Kind::OpcodeTypeFunction, parameter_types, return_type, opcode));
}


void Analyser::CreateOpcodeInstanceFunction(const TypePtr& type,
		const std::string& name,
		const std::vector<TypePtr>& parameter_types,
		const TypePtr& return_type,
		const Opcode opcode)
{
	AddFunction(type->symbols, CreateOpcodeFunction(name,
		Function::Kind::OpcodeInstanceFunction, parameter_types, return_type, opcode));
}
