...
void VM::Handler__ArrayMul()
{
  AnyInteger size;
  size.Construct(Pop());
  if (!value_util::IsAnyOf(size.type_id, TypeIds::UInt8, TypeIds::UInt16, TypeIds::UInt32,
                           TypeIds::UInt64) &&
      !ApplyFunctor<TypeIds::Int8, TypeIds::Int16, TypeIds::Int32, TypeIds::Int64>(
          size.type_id, [](auto &&value) { return value.Value() >= 0; }, size))
  {
  ...
  ApplyIntegralFunctor(size.type_id,
