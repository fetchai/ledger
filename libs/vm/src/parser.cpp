...
BlockNodePtr Parser::Parse(const std::string& source, std::vector<std::string>& errors)
{
	...
	BlockNodePtr root =
		std::make_shared<BlockNode>(Node::Kind::Root, &tokens_[0]);
	ParseBlock(*root);
...
NodePtr Parser::ParseIfStatement()
{
	...
	NodePtr if_statement_node =
		std::make_shared<Node>(Node(Node::Kind::IfStatement, token_));
	do
	{
		if (token_->kind == Token::Kind::If)
		{
			BlockNodePtr if_node =
				std::make_shared<BlockNode>(BlockNode(Node::Kind::If, token_));
		...
		{
			BlockNodePtr elseif_node =
				std::make_shared<BlockNode>(BlockNode(Node::Kind::ElseIf, token_));
			ExpressionNodePtr expression_node = ParseConditionalExpression();
			if (expression_node == nullptr)
				return nullptr;
		...
		{
			BlockNodePtr else_node =
				std::make_shared<BlockNode>(BlockNode(Node::Kind::Else, token_));
...
NodePtr Parser::ParseReturnStatement()
{
	...
	NodePtr return_statement_node =
		std::make_shared<Node>(Node(Node::Kind::ReturnStatement, token_));
	Next();
	if (token_->kind == Token::Kind::SemiColon)
...
NodePtr Parser::ParseBreakStatement()
{
	...
	NodePtr break_statement_node =
		std::make_shared<Node>(Node(Node::Kind::BreakStatement, token_));
	Next();
	if (token_->kind != Token::Kind::SemiColon)
	{
		AddError("expected ';'");
...
{
	while ((token_->kind != Token::Kind::EndOfInput) &&
		(token_->kind != Token::Kind::EndFunction))
	{
		Next();
	}
}
...
ExpressionNodePtr Parser::ParseExpression(const bool is_conditional_expression)
{
	...
	do
	{
		...
		switch (token_->kind)
		{
			...
			{
				if (HandleBinaryOp(Node::Kind::GreaterThanOrEqualOp, OpInfo(4, Association::Left, 2)) == false)
					return nullptr;
				break;
			}
...
bool Parser::HandleCloser(const bool is_conditional_expression)
{
	...
	if (token_->kind == Token::Kind::RightRoundBracket)
	{
		if ((group_kind == Node::Kind::SquareBracketGroup) ||
			(group_kind == Node::Kind::IndexOp))
		{
