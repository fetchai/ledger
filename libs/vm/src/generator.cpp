...
void Generator::HandleIfStatement(const NodePtr& node)
{
	...
	for (int i=0; i<=last_index; ++i)
	{
		...
		if (block->kind != Node::Kind::Else)
		{
			...
			ExpressionNodePtr condition_expression =
				ConvertToExpressionNodePtr(block->children[0]);
...
void Generator::HandleLHSExpression(const ExpressionNodePtr& lhs,
...
void Generator::HandleIncDecOp(const ExpressionNodePtr& node)
{
	...
	if (node->kind == Node::Kind::PrefixIncOp)
	{
		opcode_override = assigning_to_variable ? Opcode::PrefixIncOp :
			Opcode::IndexedPrefixIncOp;
	}
	else if (node->kind == Node::Kind::PrefixDecOp)
	{
		opcode_override = assigning_to_variable ? Opcode::PrefixDecOp :
			Opcode::IndexedPrefixDecOp;
	}
	else if (node->kind == Node::Kind::PostfixIncOp)
	{
		opcode_override = assigning_to_variable ? Opcode::PostfixIncOp :
			Opcode::IndexedPostfixIncOp;
	}
	else if (node->kind == Node::Kind::PostfixDecOp)
	{
		opcode_override = assigning_to_variable ? Opcode::PostfixDecOp :
			Opcode::IndexedPostfixDecOp;
	}
