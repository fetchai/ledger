...
void BuildBasicBlock(std::string const &custom_name, pybind11::module &module)
{
  ...
  py::class_<BasicBlock<P, H>, std::enable_shared_from_this<BasicBlock<P, H>>>(
      module, custom_name)
      .def(py::init<>()) /* No constructors found */
      .def("body", &BasicBlock<P, H>::body)
      .def("block_number", &BasicBlock<P, H>::block_number)
      .def("SetBody", &BasicBlock<P, H>::SetBody)
      .def("weight", (const double &(BasicBlock<P, H>::*)() const) &
                         BasicBlock<P, H>::weight)
      .def("weight",
           (double &(BasicBlock<P, H>::*)()) & BasicBlock<P, H>::weight)
      ...
      .def("total_weight", (const double &(BasicBlock<P, H>::*)() const) &
                               BasicBlock<P, H>::total_weight)
      .def("total_weight",
           (double &(BasicBlock<P, H>::*)()) & BasicBlock<P, H>::total_weight)
      .def("proof",
           (const typename fetch::chain::BasicBlock<P, H>::proof_type &(
               BasicBlock<P, H>::*)() const) &
               BasicBlock<P, H>::proof)
      .def("proof", (typename fetch::chain::BasicBlock<P, H>::proof_type &
                     (BasicBlock<P, H>::*)()) &
                        BasicBlock<P, H>::proof)
