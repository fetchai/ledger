...
void BuildMatrix(std::string const &custom_name, pybind11::module &module) {
  ...
  py::class_<Matrix< T >, fetch::math::RectangularArray<T>>(module, custom_name.c_str() )
    ...
    .def("__add__", [](Matrix<T> const & b, Matrix<T> const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());
	a.Add(b,c);
	return a;
      })
    .def("__mul__", [](Matrix<T> const & b, Matrix<T> const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());         
	a.Multiply(b,c);
	return a;
      })
    .def("__sub__", [](Matrix<T> const & b, Matrix<T> const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());                 
	a.Subtract(b,c);
	return a;
      })
    .def("__div__", [](Matrix<T> const & b, Matrix<T> const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());
	a.Divide(b,c);
	return a;
      })
    .def("__add__", [](Matrix<T> const & b, T const & c) {        
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());   
	a.Add(b,c);
	return a;
      })
    .def("__mul__", [](Matrix<T> const & b, T const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());           
	a.Multiply(b,c);
	return a;
      })
    .def("__sub__", [](Matrix<T> const & b, T const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width()); 
	a.Subtract(b,c);
	return a;
      })
    .def("__div__", [](Matrix<T> const & b, T const & c) {
	Matrix<T> a;
	a.LazyResize(b.height(), b.width());         
	a.Divide(b,c);
	return a;
      })        
    .def("__iadd__", [](Matrix<T> &a, Matrix<T> const & c) {
	a.InlineAdd(c);
	return a;
      })
    .def("__imul__", [](Matrix<T> &a, Matrix<T> const & c) {
	a.InlineMultiply(c);
	return a;
      })
    .def("__isub__", [](Matrix<T> &a, Matrix<T> const & c) {
	a.InlineSubtract(c);
	return a;
      })
    .def("__idiv__", [](Matrix<T> &a, Matrix<T> const & c) {
	a.InlineDivide(c);
	return a;
      })
    .def("__iadd__", [](Matrix<T> &a, T const & c) {
	a.InlineAdd(c);
	return a;
      })
    .def("__imul__", [](Matrix<T> &a, T const & c) {
	a.InlineMultiply(c);
	return a;
      })
    .def("__isub__", [](Matrix<T> &a, T const & c) {
	a.InlineSubtract(c);
	return a;
      })
    .def("__idiv__", [](Matrix<T> &a, T const & c) {
	a.InlineDivide(c);
	return a;
      })
    .def("__len__", [](Matrix<T> const &a) {
        return a.size();
      })
