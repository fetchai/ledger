...
void BuildVersionedRandomAccessStack(std::string const &custom_name,
                                     pybind11::module & module)
{

  namespace py = pybind11;
  py::class_<VersionedRandomAccessStack<T, B>, RandomAccessStack<T, B>>(
      module, custom_name)
      .def(py::init<>()) /* No constructors found */
      .def("Load", &VersionedRandomAccessStack<T, B>::Load)
      .def("Set", &VersionedRandomAccessStack<T, B>::Set)
      .def("Get", (typename RandomAccessStack<T>::type(
                      VersionedRandomAccessStack<T, B>::*)(const std::size_t &)
                       const) &
                      VersionedRandomAccessStack<T, B>::Get)
      .def(
          "Get",
          (void (VersionedRandomAccessStack<T, B>::*)(
              const std::size_t &,
              typename fetch::storage::VersionedRandomAccessStack<T, B>::type &)
               const) &
              VersionedRandomAccessStack<T, B>::Get)
      ...
      .def("PreviousBookmark",
           &VersionedRandomAccessStack<T, B>::PreviousBookmark)
