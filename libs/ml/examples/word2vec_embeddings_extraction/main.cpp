...
int main(int argc, char **argv)
{
  ...
  const Vocab &vcb         = *(data_loader.GetVocab());
  std::string  knn_results = utilities::KNNTest(vcb, weights, "three", 20);
  std::cout << std::endl << knn_results << std::endl;

  std::string word_analogy_results =
      utilities::WordAnalogyTest(vcb, weights, "king", "queen", "father", 20);
  std::cout << std::endl << word_analogy_results << std::endl;

  auto analogies_file_results = utilities::AnalogiesFileTest(vcb, weights, analogy_file);
