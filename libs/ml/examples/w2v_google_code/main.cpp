...
real
  alpha = float(0.025),               // linear-decay learning rate
...
int GetWordHash(char *word) {
  ...
  for (a = 0; a < strlen(word); a++) hash = hash * 257 + static_cast<unsigned long long>(word[a]);
...
int AddWordToVocab(char *word) {
  ...
  if (vocab_size + 2 >= vocab_max_size) {
    vocab_max_size += 1000;
    vocab = (struct vocab_word *)realloc(vocab, static_cast<unsigned long long>(vocab_max_size) * sizeof(struct vocab_word));
  }
...
void SortVocab() {
  ...
  vocab = (struct vocab_word *)realloc(vocab, static_cast<unsigned long long>(vocab_size + 1) * sizeof(struct vocab_word));
...
void CreateBinaryTree() {
  ...
  long long *count = (long long *)calloc(static_cast<size_t>(vocab_size * 2 + 1), sizeof(long long));
  long long *binary = (long long *)calloc(static_cast<size_t>(vocab_size * 2 + 1), sizeof(long long));
  long long *parent_node = (long long *)calloc(static_cast<size_t>(vocab_size * 2 + 1), sizeof(long long));
...
void InitNet() {
  ...
  a = posix_memalign((void **)&syn0, 128, static_cast<unsigned long long>(vocab_size * layer1_size) * sizeof(real));
  if (syn0 == NULL) {printf("Memory allocation failed\n"); exit(1);}
  if (hs) {
    a = posix_memalign((void **)&syn1, 128, static_cast<unsigned long long>(vocab_size * layer1_size) * sizeof(real));
    if (syn1 == NULL) {printf("Memory allocation failed\n"); exit(1);}
    for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++)
     syn1[a * layer1_size + b] = 0;
  }
  if (negative>0) {
    a = posix_memalign((void **)&syn1neg, 128, static_cast<unsigned long long>(vocab_size * layer1_size) * sizeof(real));
  ...
  for (a = 0; a < vocab_size; a++) for (b = 0; b < layer1_size; b++) {
    ...
    syn0[a * layer1_size + b] = ((static_cast<float>(static_cast<int>(next_random) & 0xFFFF) / static_cast<float>(65536)) - static_cast<float>(0.5)) / layer1_size;
  }
  CreateBinaryTree();
}
...
void *TrainModelThread(void *id) {
  ...
  unsigned long long
    next_random = (unsigned long long)id;  // thread-specific RNG state
  ...
  real
    *neu1 = (real *)calloc(static_cast<size_t>(layer1_size), sizeof(real)),
  ...
  while (1) {
    ...
    if (word_count - last_word_count > 10000) {
      ...
      if (alpha < starting_alpha * 0.0001) alpha = static_cast<float>(starting_alpha * 0.0001);
    }
      ...
      if (cw) {
        ...
        if (negative > 0) for (d = 0; d < negative + 1; d++) {
          if (d == 0) {
            ...
            if (target == 0) target = static_cast<long long>(next_random % static_cast<unsigned long long>(vocab_size - 1) + 1);
...
int main(int argc, char **argv) {
  ...
  if (cbow) alpha = static_cast<float>(0.05);
  if ((i = ArgPos((char *)"-alpha", argc, argv)) > 0) alpha = static_cast<float>(atof(argv[i + 1]));
  if ((i = ArgPos((char *)"-output", argc, argv)) > 0) strcpy(output_file, argv[i + 1]);
  if ((i = ArgPos((char *)"-window", argc, argv)) > 0) window = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-sample", argc, argv)) > 0) sample = static_cast<float>(atof(argv[i + 1]));
  ...
  vocab = (struct vocab_word *)calloc(static_cast<unsigned long>(vocab_max_size), sizeof(struct vocab_word));
