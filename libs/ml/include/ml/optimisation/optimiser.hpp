 ...
 */
            ...
            void Optimiser<T, C>::PrintStats(SizeType batch_size, SizeType subset_size)
            {
                ...
                {
                    stat_string_ = std::to_string(step_) + " (??%) -- " + "learning rate: " + std::to_string(static_cast<double>(learning_rate_)) + " -- " +  std::to_string(static_cast<double>(step_) / static_cast<double>(time_span_.count())) + " samples / sec ";
                }
                else
                {
                    stat_string_ =  std::to_string(step_) + " / " +  std::to_string(subset_size) + " (" +  std::to_string(static_cast<SizeType>(100.0 * static_cast<double>(step_) / static_cast<double>(subset_size))) + "%) -- " + "learning rate: " +  std::to_string(static_cast<double>(learning_rate_)) + " -- " +  std::to_string(static_cast<double>(step_) / static_cast<double>(time_span_.count())) + " samples / sec ";
                }
                // print it in log
//    FETCH_LOG_INFO("ML_LIB", "Training speed", stat_string_);
//    FETCH_LOG_INFO("ML_LIB", "Batch loss: ", loss_sum / step);
//      // print it in cout
                std::cout << "\r" <<  stat_string_ << " batch loss: " << loss_sum_ / static_cast<DataType>(step_ / batch_size) << std::flush;
            }
/**
 *
 * @tparam T
