...
public:
  ...
  std::string positionwise_feedforward(std::string name, std::string const &input){
	  ...
	  // do dropout
	  std::string ff_first_layer_dropout = this->template AddNode<fetch::ml::ops::Dropout<ArrayType>>(name + "_Dropout", {ff_first_layer}, feedforward_dropout_);
  ...
  std::string residual_connection(std::string name, std::string const &prev_layer_input, std::string const &prev_layer_output){
  	...
  	std::string dropout_output = this->template AddNode<fetch::ml::ops::Dropout<ArrayType>>(name + "_Dropout", {prev_layer_output}, residual_dropout_);
	  std::string residual_addition = this->template AddNode<fetch::ml::ops::Add<ArrayType>>(name + "_Residual_Addition", {prev_layer_input, dropout_output});

		// create sudo shape for layernorm
		std::vector<SizeType> data_shape({model_dim_, 1});
  	std::string normalized_residual = this->template AddNode<fetch::ml::layers::LayerNorm<ArrayType>>(name + "_LayerNorm", {residual_addition}, data_shape, static_cast<SizeType>(0));
  	
  	return normalized_residual;
  }

  SelfAttentionEncoder(SizeType n_heads, SizeType model_dim, SizeType ff_dim, DataType residual_dropout = static_cast<DataType>(0.9), DataType attention_dropout = static_cast<DataType>(0.9), DataType feedforward_dropout = static_cast<DataType>(0.9))
    : n_heads_(n_heads)
    ...
    , feedforward_dropout_(feedforward_dropout)
  {
    ...
    std::string attention_residual = residual_connection(name + "_Attention_Residual", input, multihead_self_attention);
    
    // do feed forward
    std::string feedforward = positionwise_feedforward(name + "_Feedforward", attention_residual);
    
    // make residual connection
    std::string feedforward_residual = residual_connection(name + "_Feedforward_Residual", attention_residual, feedforward);
