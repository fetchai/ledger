...
public:
  ...
  FullyConnected(NodePtrType target_node){
	  ...
	  // TODO () after Emma's change on state dict is merged, we should be able to set up regularization easily, currently we do not set regularizor
	  this->AddInputNode(input);
	  this->SetOutputNode(output);
	  this->SetRegularisation(fetch::ml::details::CreateRegulariser<T>(fetch::ml::details::RegularisationType::NONE), static_cast<DataType>(0));
	  ...
	  NodePtrType weights_ptr = target_node->GetNode(name + "_Weights");
	  NodePtrType bias_ptr    = target_node->GetNode(name + "_Bias");
