...
void BM_TensorDataLoader_AddData(benchmark::State &state)
{
  ...
  state.counters["charge"] = static_cast<double>(dl.ChargeAddData({data}, labels));

  for (auto _ : state)
  {
    // Initialise Optimiser
    dl.AddData({data}, labels);
  }
}
...
static void TensorDataLoaderAddDataArguments(benchmark::internal::Benchmark *b)
{
  ...
  SizeType n_benchmarks = data_sizes.size();
  for (SizeType i = 0; i < n_benchmarks; i++)
  {
    b->Args({data_sizes[i], input_sizes[i], output_sizes[i]});
  }
}

BENCHMARK_TEMPLATE(BM_TensorDataLoader_AddData,
    float)
...
->Unit(::benchmark::kMicrosecond);
BENCHMARK_TEMPLATE(
    BM_TensorDataLoader_AddData,
    fetch::fixed_point::fp32_t)
->Apply(TensorDataLoaderAddDataArguments)
->Unit(::benchmark::kMicrosecond);
BENCHMARK_TEMPLATE(
    BM_TensorDataLoader_AddData,
...
BENCHMARK_TEMPLATE(BM_TensorDataLoader_GetNext,
                   float)
