...
#include "ml/layers/depthwise_conv2d.hpp"
...
TYPED_TEST_CASE(DepthwiseConv2DTest, math::test::TensorFloatingTypes);

TYPED_TEST(DepthwiseConv2DTest, set_input_and_evaluate_test)  // Use the class as a subgraph
{
  ...
  layers::DepthwiseConv2D<TensorType> c;
...
TYPED_TEST(DepthwiseConv2DTest, ops_forward_test)  // Use the class as an Ops
{
  using DataType   = typename TypeParam::Type;
  using TensorType = TypeParam;
  using SizeType   = fetch::math::SizeType;
...
TYPED_TEST(DepthwiseConv2DTest, ops_backward_test)  // Use the class as an Ops
{
  using DataType   = typename TypeParam::Type;
  using TensorType = TypeParam;
  using SizeType   = fetch::math::SizeType;
  ...
  auto                                weights = conv.GetWeights();
  layers::DepthwiseConv2D<TensorType> op;
...
TYPED_TEST(DepthwiseConv2DTest, node_forward_test)  // Use the class as a Node
{
  using DataType   = typename TypeParam::Type;
  using TensorType = TypeParam;
  using SizeType   = fetch::math::SizeType;
  ...
  auto conv = fetch::ml::Node<TensorType>(
      fetch::ml::OpType::LAYER_CONVOLUTION_2D, "DepthwiseConv2D",
      [output_channels, input_channels, kernel_height, stride_size]() {
        return std::make_shared<fetch::ml::layers::DepthwiseConv2D<TensorType>>(
            output_channels, input_channels, kernel_height, stride_size);
      });
...
TYPED_TEST(DepthwiseConv2DTest, node_backward_test)  // Use the class as a Node
{
  using DataType   = typename TypeParam::Type;
  using TensorType = TypeParam;
  using SizeType   = fetch::math::SizeType;
  ...
  auto conv2d_layer_ptr = std::make_shared<fetch::ml::layers::DepthwiseConv2D<TensorType>>(
      output_channels, input_channels, kernel_height, stride_size);
  auto conv = fetch::ml::Node<TensorType>(
      fetch::ml::OpType::LAYER_CONVOLUTION_2D, "DepthwiseConv2D",
      [output_channels, input_channels, kernel_height, stride_size]() {
...
TYPED_TEST(DepthwiseConv2DTest, graph_forward_test)  // Use the class as a Node
{
  using DataType   = typename TypeParam::Type;
  using TensorType = TypeParam;
  using SizeType   = fetch::math::SizeType;
  ...
  g.template AddNode<fetch::ml::layers::DepthwiseConv2D<TensorType>>(
      "DepthwiseConv2D", {"Input"}, output_channels, input_channels, kernel_height, stride_size);
...
TYPED_TEST(DepthwiseConv2DTest, getStateDict)
{
  using TensorType = TypeParam;
  using DataType   = typename TypeParam::Type;
  using SizeType   = fetch::math::SizeType;
  ...
  // Initialise weights
  fetch::ml::layers::DepthwiseConv2D<TensorType> conv(
      output_channels, input_channels, kernel_height, stride_size,
...
TYPED_TEST(DepthwiseConv2DTest, saveparams_test)
{
  using TensorType = TypeParam;
  using DataType   = typename TypeParam::Type;
  using SizeType   = fetch::math::SizeType;
  using LayerType  = fetch::ml::layers::DepthwiseConv2D<TensorType>;
