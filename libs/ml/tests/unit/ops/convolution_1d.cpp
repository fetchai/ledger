...
TYPED_TEST(Convolution1DTest, forward_3x3x2_5x3x3x2)
{
  ...
  // Test correct values
  ASSERT_TRUE(output.AllClose(gt, fetch::math::AsType<DataType>(1e-5f), fetch::math::AsType<DataType>(1e-5f)));
}
...
TYPED_TEST(Convolution1DTest, backward_3x3x2_5x3x3x2)
{
  ...
  // Test correct values
  ASSERT_TRUE(prediction[0].AllClose(gt1, fetch::math::AsType<DataType>(1e-5f), fetch::math::AsType<DataType>(1e-5f)));
  ASSERT_TRUE(prediction[1].AllClose(gt2, fetch::math::AsType<DataType>(1e-5f), fetch::math::AsType<DataType>(1e-5f)));
}
...
TYPED_TEST(Convolution1DTest, saveparams_test)
{
  ...
  // test correct values
  EXPECT_TRUE(
      new_prediction.AllClose(prediction, DataType{0}, DataType{0}));
}
