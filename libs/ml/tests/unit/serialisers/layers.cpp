...
TYPED_TEST(LayersSaveParamsTest, conv1d_saveparams_test)
{
  ...
  auto dsp    = serialiser_test_utils::SerialiseDeserialiseBuild<SPType, TensorType>(layer);
  auto layer2 = *(::fetch::ml::utilities::BuildLayer<TensorType, LayerType>(dsp));

  serialiser_test_utils::TestLayerPredictionsEqual<DataType>(layer, layer2, input, input_name,
...
TYPED_TEST(LayersSaveParamsTest, fully_connected_saveparams_test)
{
  ...
  auto dsp    = serialiser_test_utils::SerialiseDeserialiseBuild<SPType, TypeParam>(layer);
...
TYPED_TEST(LayersSaveParamsTest, prelu_saveparams_test)
{
  ...
  ::fetch::math::SizeType input_dim_0 = 5;
  ::fetch::math::SizeType input_dim_1 = 10;
  ::fetch::math::SizeType input_dim_2 = 2;
...
TYPED_TEST(LayersSaveParamsTest, skipgram_saveparams_test)
{
  ...
  // test that deserialised model gives the same forward prediction as the original layer
  layer.SetInput("SkipGram_Input", input);
  layer.SetInput("SkipGram_Context", context);
  TypeParam prediction = layer.Evaluate(output_name, true);
  ...
  //
  // test that the deserialised model gives the same result as the original layer after training
  //
