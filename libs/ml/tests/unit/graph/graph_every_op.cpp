...
void ComparePrediction(GraphPtrType g, GraphPtrType g2, std::string node_name)
{
  ...
  EXPECT_TRUE(prediction.AllClose(prediction2, fetch::math::Type<DataType>("0"),
...
TYPED_TEST(GraphRebuildTest, graph_rebuild_every_op)
{
  ...
  query_data.Fill(fetch::math::Type<DataType>("0"));
  ...
  std::string maskfill =
      AddOp<ops::MaskFill<TensorType>>(g, {input_1, input_1}, fetch::math::Type<DataType>("0"));
  ...
  std::string randomisedrelu = AddOp<ops::RandomisedRelu<TensorType>>(
      g, {input_1}, fetch::math::Type<DataType>("0"), fetch::math::Type<DataType>("1"));
