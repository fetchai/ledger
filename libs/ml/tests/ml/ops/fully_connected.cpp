...
TYPED_TEST(FullyConnectedTest, backward_test) // Use the class as an Ops
{
  ...
  std::shared_ptr<TypeParam> inputData = std::make_shared<TypeParam>(std::vector<size_t>({5, 10}));
  std::shared_ptr<TypeParam> output = fc.Forward({inputData});
  std::shared_ptr<TypeParam> errorSignal = std::make_shared<TypeParam>(std::vector<size_t>({1, 10}));

  std::vector<std::shared_ptr<TypeParam>> backpropagatedErrorSignals = fc.Backward({inputData}, errorSignal);
