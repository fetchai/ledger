...
typename ArrayType::Type TrainOneBatch(fetch::ml::Graph<ArrayType> &g,
        ArrayType &data,
        ArrayType &gt,
        ...
        typename ArrayType::Type alpha)
{
  ...
  ArrayType cur_gt{{1, 1}};
  ArrayType cur_input{{1, gt.shape().at(1)}};
...
TYPED_TEST(FullyConnectedTest, xor_mse_test)  // Use the class as a Node
{
  ...
  std::string fc1_name = g.template AddNode<fetch::ml::layers::FullyConnected<TypeParam>>("FC1", {input_name}, input_size, 10u);
  std::string relu_name = g.template AddNode<fetch::ml::ops::Relu<TypeParam>>("relu", {fc1_name});
  std::string fc2_name = g.template AddNode<fetch::ml::layers::FullyConnected<TypeParam>>("FC2", {relu_name}, 10u, 10u);
  std::string relu2_name = g.template AddNode<fetch::ml::ops::Relu<TypeParam>>("relu2", {fc2_name});
  std::string fc3_name = g.template AddNode<fetch::ml::layers::FullyConnected<TypeParam>>("FC3", {relu_name}, 10u, output_size);
  std::string output_name = g.template AddNode<fetch::ml::ops::Sigmoid<TypeParam>>("sigmoid", {fc3_name});
...
TYPED_TEST(FullyConnectedTest, xor_sce_test)  // Use the class as a Node
{
  ...
  std::string input_name = g.template AddNode<fetch::ml::ops::PlaceHolder<TypeParam>>("Input", {});
  std::string fc1_name = g.template AddNode<fetch::ml::layers::FullyConnected<TypeParam>>("FC1", {input_name}, input_size, 100u);
  std::string sig_name = g.template AddNode<fetch::ml::ops::Sigmoid<TypeParam>>("sigmoid", {fc1_name});
  std::string output_name = g.template AddNode<fetch::ml::layers::FullyConnected<TypeParam>>("FC2", {sig_name}, 100u, output_size);
