...
TYPED_TEST(ScaledDotProductAttention,
           self_attention_backward_exact_value_check)  // Use the class as a layer
{
	using DataType = typename TypeParam::Type;
	using SizeType = typename TypeParam::SizeType;
	
	fetch::ml::layers::ScaledDotProductAttention<TypeParam> att(static_cast<SizeType>(3), DataType(0.1));
	
	TypeParam query_data = TypeParam::FromString("1, 2, 0.5, 0.1; 2, 1, 0.3, -0.2;2, 4, 0, 1");
	query_data.Reshape({3, 2, 2});
	
	TypeParam output(att.ComputeOutputShape({std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data)}));
	att.Forward({std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data)}, output);
	...
	TypeParam gt_query_grad = TypeParam::FromString(
	 "0.1474872519,  0.0094864446, -0.4040479300,  0.0737092770; -0.1474872519, -0.0094864446, -0.5050599125,  0.0921365963; 0.2949745039,  0.0189728892,  1.0101198249, -0.1842731926");
	gt_query_grad.Reshape({3, 2, 2});
	TypeParam gt_key_grad = TypeParam::FromString(
	 "-0.1664601411,  0.1664601411, -0.4866325932,  0.4866325932; -0.3044609485,  0.3044609485, -0.3398905860,  0.3398905860; -0.3329202822,  0.3329202822,  0.1842731926, -0.1842731926");
	gt_key_grad.Reshape({3, 2, 2});
	TypeParam gt_value_grad = TypeParam::FromString(
	 "0.1558327790,  1.8441672210, -0.4173382500, -0.8826617500; 0.1668474430,  3.8331525570,  1.2413162873,  2.4586837127; 0.1640937770,  3.3359062230,  3.8524286190,  3.1475713810");
	gt_value_grad.Reshape({3, 2, 2});
	
	// do the backprop
	std::vector<TypeParam> backprop_error =
	 att.Backward({std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data)}, error_signal);
	
	ASSERT_TRUE(backprop_error[0].AllClose(
	 gt_query_grad, static_cast<DataType>(15) * fetch::math::function_tolerance<DataType>()));
	ASSERT_TRUE(backprop_error[1].AllClose(
	 gt_key_grad, static_cast<DataType>(15) * fetch::math::function_tolerance<DataType>()));
	ASSERT_TRUE(backprop_error[2].AllClose(
	 gt_value_grad, static_cast<DataType>(15) * fetch::math::function_tolerance<DataType>()));
}
