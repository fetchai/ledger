...
//	mask_data.Fill(static_cast<DataType>(1));
//
//	TypeParam error_signal = TypeParam::FromString("1, 1, 0.2, -1.5; 1, 3, -0.3, 4; 1, 2.5, 7,
// 0"); 	error_signal.Reshape({3, 2, 2});
//
//	TypeParam gt_query_grad = TypeParam::FromString(
//	 "0.1474872519,  0.0094864446, -0.4040479300,  0.0737092770; -0.1474872519, -0.0094864446, "
//	 "-0.5050599125,  0.0921365963; 0.2949745039,  0.0189728892,  1.0101198249, -0.1842731926");
...
//	 "-0.1664601411,  0.1664601411, -0.4866325932,  0.4866325932; -0.3044609485,  0.3044609485,
//"
//	 "-0.3398905860,  0.3398905860; -0.3329202822,  0.3329202822,  0.1842731926,
//-0.1842731926"); 	gt_key_grad.Reshape({3, 2, 2}); 	TypeParam gt_value_grad =
//TypeParam::FromString( 	 "0.1558327790,  1.8441672210, -0.4173382500, -0.8826617500; 0.1668474430,
//3.8331525570,  " 	 "1.2413162873,  2.4586837127; 0.1640937770,  3.3359062230,  3.8524286190,
//3.1475713810"); 	gt_value_grad.Reshape({3, 2, 2}); 	TypeParam gt_mask_grad({1, 2, 2});
//
//	// do the forward
...
//	att.Forward({std::make_shared<TypeParam>(query_data),
// std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data),
// std::make_shared<TypeParam>(mask_data)}, 	            output);
//
//	// do the backprop
//	std::vector<TypeParam> backprop_error = att.Backward(
//	 {std::make_shared<TypeParam>(query_data), std::make_shared<TypeParam>(query_data),
