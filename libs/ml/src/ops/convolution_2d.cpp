...
template <class TensorType>
math::SizeType Convolution2D<TensorType>::ComputeOutputDim(SizeType const input_dim,
    ...
    std::vector<math::SizeVector> const &input_shapes)
{
  ...
  SizeType batch_size      = input_shapes.front().at(3);
  SizeType output_channels = input_shapes.back().at(0);
  SizeType kernel_height   = input_shapes.back().at(2);
  SizeType kernel_width    = input_shapes.back().at(3);

  SizeType output_height = ComputeOutputDim(input_shapes.front().at(1), input_shapes.back().at(2));

  SizeType output_width = ComputeOutputDim(input_shapes.front().at(2), input_shapes.back().at(3));
