...
public:
  ...
  using ProtoPipeBuilder = std::function<std::shared_ptr<IN_PROTO>(std::shared_ptr<OUT_PROTO>, std::shared_ptr<MIDDLE_PROTO>)>;
  using ErrorHandler     = std::function<void(const std::string&, const std::string&, const std::string&)>;
  ...
  DapSerialConversationTask(
    ...
    :  StateMachineTask<DapSerialConversationTask>(this,  {&DapSerialConversationTask::progress,
                                                           &DapSerialConversationTask::progress
  })
    , msg_id_{msg_id}
  ...
  StateResult progress()
  {
    ...
    auto dapTask = std::make_shared<DapConversationTask<IN_PROTO, OUT_PROTO>>(data.dap_name, data.path, msg_id_, input, outbounds, endpoint);
    ...
    if (dapTask-> makeNotification().Then( [this_wp](){ auto sp = this_wp.lock(); if (sp) { sp -> makeRunnable(); } } ).Waiting())
    {
...
private:
  ...
  DapSerialConversationTask &operator=(const DapSerialConversationTask &other) = delete; // { copy(other); return *this; }
  bool operator==(const DapSerialConversationTask &other) = delete; // const { return compare(other)==0; }
  bool operator<(const DapSerialConversationTask &other) = delete; // const { return compare(other)==-1; }
};
