            ...
            case RBCMessage::MessageType::RBROADCAST:
            {
                ...
                if (broadcast_ptr != nullptr)
                {
                    if (Failure(Failures::BAD_MESSAGE))
                    {
                        ...
                        serialiser << new_msg;
                        RBroadcast new_broadcast{CHANNEL_BROADCAST, broadcast_ptr->id(), broadcast_ptr->counter(),
                                                 serialiser.data()};
                        FETCH_LOG_INFO(LOGGING_NAME, "Node: ", id_, " received RBroadcast from node ",
                                       sender_index, " with (tag, hash): (", new_broadcast.tag(), ",",
                    ...
                    {
                        FETCH_LOG_INFO(LOGGING_NAME, "Node: ", id_, " received RBroadcast from node ",
                                       sender_index, " with (tag, hash): (", broadcast_ptr->tag(), ",",
                                       MessageHash(broadcast_ptr->message()), ")");
                     ...
                     const std::vector<std::vector<FaultyRbc::Failures>> &failures = {})
{
    ...
    // First node sends a broadcast
    {
        ...
        committee[sender_index]->rbc.SendRBroadcast(serialiser.data());

        std::this_thread::sleep_for(std::chrono::seconds(25));
        uint32_t pp = 0;
        for (auto qq = 0; qq < cabinet_size; ++qq)
        {
            if (qq != sender_index && committee[qq]->delivered_msgs == 1)
            {
                ++pp;
            }
        }
...
{
    // Three node withhold their echo messages and no one should received enough to deliver the message
    GenerateRbcTest(4, 0, {{FaultyRbc::Failures::NO_ECHO}, {FaultyRbc::Failures::NO_ECHO}, {FaultyRbc::Failures::NO_ECHO}});
}
...
{
    // Three node withhold their ready messages and no one should received enough to deliver the message
    GenerateRbcTest(4, 0, {{FaultyRbc::Failures::NO_READY}, {FaultyRbc::Failures::NO_READY},{FaultyRbc::Failures::NO_READY}});
}
...
TEST(rbc, too_many_no_answer)
{
    ...
    GenerateRbcTest(4, 3, {{FaultyRbc::Failures::BAD_MESSAGE}, {FaultyRbc::Failures::NO_ANSWER}, {FaultyRbc::Failures::NO_ANSWER}, {FaultyRbc::Failures::NO_ANSWER}});
}