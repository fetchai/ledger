 ...
 */
        void DkgSetupService::BroadcastShares()
        {
            ...
            for (auto &cab_i : cabinet_)
            {
                if (cab_i != address_)
                {
                    std::pair<MessageShare, MessageShare> shares{manager_.GetShares(cab_i)};
 ...
 */
        void DkgSetupService::BroadcastComplaintsAnswer()
        {
            ...
            for (auto const &reporter : complaints_manager_.ComplaintsFrom())
            {
                ...
                FETCH_LOG_INFO(LOGGING_NAME, "Node ", manager_.cabinet_index(), " received complaints from ", from_index);
                complaints_answer.insert({reporter, manager_.GetShares(reporter)});
            }
            SendBroadcast(
 ...
 */
        void DkgSetupService::BroadcastQualCoefficients()
        {
            SendBroadcast(DKGEnvelope{CoefficientsMessage{
                    static_cast<uint8_t>(State::WAITING_FOR_QUAL_SHARES), manager_.GetQualCoefficients(), "signature"}});
 ...
 */
        ...
        void DkgSetupService::BroadcastReconstructionShares()
        {
            ...
            for (auto const &in : qual_complaints_manager_.Complaints())
            {
                ...
                complaint_shares.insert({in, manager_.GetShares(in)});
            }
            SendBroadcast(
 ...
 */
        void DkgSetupService::ReceivedComplaint()
        {
            ...
            if (!received_all_complaints_ && state_ == State::WAITING_FOR_COMPLAINTS &&
                complaints_manager_.IsFinished(cabinet_, manager_.cabinet_index(), manager_.polynomial_degree()))
            {
 ...
 */
        void DkgSetupService::ReceivedComplaintsAnswer()
        {
                ...
                complaints_answer_manager_.IsFinished(cabinet_, manager_.cabinet_index()))
            {
                ...
                if (BuildQual())
                {
                    FETCH_LOG_INFO(LOGGING_NAME, "Node ", manager_.cabinet_index(), "build qual of size ", qual_.size());
 ...
 */
        void DkgSetupService::ReceivedQualComplaint()
        {
                ...
                (qual_complaints_manager_.IsFinished(qual_, address_)))
            {
                ...
                {
                    FETCH_LOG_WARN(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " DKG has failed: complaints size ",
                                   size);
                    state_ = State::FINAL;
                    dkg_state_gauge_->set(static_cast<uint8_t>(state_.load()));
                    lock.unlock();
 ...
 */
        void DkgSetupService::OnDkgMessage(MuddleAddress const &       from,
                                                    std::shared_ptr<DKGMessage> msg_ptr)
        {
            ...
            switch (msg_ptr->type())
            {
                ...
                default:
                    FETCH_LOG_ERROR(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " can not process payload from node ",
                                    senderIndex);
            }
        }
 ...
 */
        void DkgSetupService::OnExposedShares(SharesMessage const &shares,
                                                       MuddleAddress const &from_id)
        {
            ...
            {
                FETCH_LOG_INFO(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " received complaint answer from ",
            ...
            {
                FETCH_LOG_INFO(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " received QUAL complaint from ",
                               CabinetIndex(from_id));
                OnQualComplaints(shares, from_id);
            }
            else if (phase1 == static_cast<uint64_t>(State::WAITING_FOR_RECONSTRUCTION_SHARES))
            {
                FETCH_LOG_INFO(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " received reconstruction share from ",
                               CabinetIndex(from_id));
 ...
 */
        ...
        {
            manager_.AddShares(from, shares);
            ++shares_received_;
            ReceivedCoefficientsAndShares();
        }
 ...
 */
                                                         ...
                                                         MuddleAddress const &      from_id)
        {
            ...
            {
                manager_.AddCoefficients(from_id, msg.coefficients());
                ++C_ik_received_;
                ReceivedCoefficientsAndShares();
            }
            else if (msg.phase() == static_cast<uint64_t>(State::WAITING_FOR_QUAL_SHARES))
            {
                manager_.AddQualCoefficients(from_id, msg.coefficients());
                {
                    std::unique_lock<std::mutex> lock{mutex_};
 ...
 * @param from_id Muddle address of sender
 */
        void DkgSetupService::OnComplaints(ComplaintsMessage const &msg,
                                                    MuddleAddress const &    from_id)
        {
 ...
 * @param from_id Muddle address of sender
 */
        void DkgSetupService::OnComplaintsAnswer(SharesMessage const &answer,
                                                          MuddleAddress const &from_id)
        {
 ...
 */
        bool DkgSetupService::BuildQual()
        {
            ...
            {
                FETCH_LOG_WARN(LOGGING_NAME, "Node: ", manager_.cabinet_index(), " build QUAL failed as not in QUAL");
                return false;
            }
 ...
 */
                                                     ...
                                                     std::shared_ptr<DKGMessage> const &msg_ptr)
        {
            ...
            {
                FETCH_LOG_WARN(LOGGING_NAME, "Node ", manager_.cabinet_index(), " received message from unknown sender");
                return false;
            }
            return true;
        }
