...
namespace fetch {
    namespace dkg {
        ...
        DkgManager::DkgManager(MuddleAddress const &address):address_{address}  {
        ...
        std::pair<DkgManager::Share, DkgManager::Share> DkgManager::GetShares(MuddleAddress const &from)
        {
            std::pair<Share, Share> shares_j {s_ij[cabinet_index_][identity_to_index_[from]].getStr(),
                                                          sprime_ij[cabinet_index_][identity_to_index_[from]].getStr()};
            return shares_j;
        }

        void DkgManager::AddCoefficients(MuddleAddress const &from, std::vector<Coefficient> const &coefficients)
        {
        ...
        void DkgManager::AddShares(MuddleAddress const &from, std::pair<Share, Share> const &shares) {
            ...
            s_ij[identity_to_index_[from]][cabinet_index_].setStr(shares.first);
            sprime_ij[identity_to_index_[from]][cabinet_index_].setStr(shares.second);
        }
 ...
 */
        std::unordered_set<DkgManager::MuddleAddress>
        DkgManager::ComputeComplaints()
        {
        ...
        bool DkgManager::VerifyComplaintAnswer(MuddleAddress const &from, ComplaintAnswer const &answer)
        {
            ...
            if (lhsG != rhsG)
            {
                FETCH_LOG_WARN(LOGGING_NAME, "Node: ", cabinet_index_, " verification for node ",
                               from_index, " complaint answer failed");
                return false;
            }
            else
            {
                FETCH_LOG_INFO(LOGGING_NAME, "Node: ", cabinet_index_, " verification for node ",
                               from_index, " complaint answer succeeded");
 ...
 */
        ...
        DkgManager::MuddleAddress DkgManager::VerifyQualComplaint(MuddleAddress const &from, ComplaintAnswer const &answer) {
            uint32_t from_index = identity_to_index_[from];
...
*/
        ...
        void DkgManager::ComputePublicKeys(std::set<MuddleAddress> const &qual) {
                ...
                for (auto const &iq : qual)
                {
                    ...
                    y_i[it] = A_ik[identity_to_index_[iq]][0];
                }
            ...
            uint32_t from_index = identity_to_index_[from];
                if (reconstruction_shares.at(share_owner).second[from_index] != zeroFr_) {
                    FETCH_LOG_WARN(LOGGING_NAME, "Node ", cabinet_index_,
        ...
        void DkgManager::AddReconstructionShare(MuddleAddress const &from, std::pair<MuddleAddress, Share> const &share) {
            ...
            if (reconstruction_shares.find(share.first) == reconstruction_shares.end())
            {
                reconstruction_shares.insert(
                        {share.first, {{}, std::vector<bn::Fr>(cabinet_size_, zeroFr_)}});
            }
