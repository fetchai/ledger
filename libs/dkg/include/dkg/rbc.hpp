    ...
    namespace dkg {
        namespace rbc {
            ...
            private:
                ...
                enum class MsgType : uint8_t { R_SEND, R_ECHO, R_READY, R_REQUEST, R_ANSWER };
                static std::string msgType_to_string(MsgType m);
                ...
                struct Party { // information per party
                    std::unordered_map<TagType,std::bitset<sizeof(MsgType) * 8>> flags_;
                    uint8_t deliver_s_;
                ...
                bool setMbar(TagType tag, std::shared_ptr<RMessage> msg_ptr, uint32_t l);
                ...
                bool setPartyFlag(uint32_t l, TagType tag, MsgType m);
