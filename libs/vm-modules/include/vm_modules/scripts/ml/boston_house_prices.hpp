...
std::string BostonHousingScript(std::string batch_size = "8u64", bool load_data = false)
{
  ...
  std::string batch_script      = R"(
      var batch_size =)";
  std::string batch_size_script = batch_size;
  ...
  ss << script << data_script << model_script << batch_script << batch_size_script << end_line
     << work_script;
