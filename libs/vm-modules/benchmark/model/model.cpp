...
void BM_AddActivation(::benchmark::State &state)
{
  ...
  for (auto _ : state)
  {
      for (auto activation : activations_) {
          ...
          state.counters["charge"] = static_cast<double>(model->Estimator().LayerAddActivation(layer_type, activation_type));
          state.counters["ForwardCost"] = static_cast<double>(model->Estimator().GetForwardCost());
          state.counters["OpsCount"]    = static_cast<double>(model->Estimator().GetOpsCount());
          state.counters["PaddedSizesSum"] = static_cast<double>(model->Estimator().GetPaddedSizesSum());
          state.counters["SizesSum"]       = static_cast<double>(model->Estimator().GetSizesSum());
...
BENCHMARK(BM_AddActivation)->Args({1, 1})->Unit(::benchmark::kMicrosecond);
BENCHMARK(BM_AddActivation)->Args({10, 1})->Unit(::benchmark::kMicrosecond);
BENCHMARK(BM_AddActivation)->Args({100, 1})->Unit(::benchmark::kMicrosecond);
BENCHMARK(BM_AddActivation)->Args({1000, 1})->Unit(::benchmark::kMicrosecond);
BENCHMARK(BM_AddActivation)->Args({10, 10})->Unit(::benchmark::kMicrosecond);
BENCHMARK(BM_AddActivation)->Args({100, 10})->Unit(::benchmark::kMicrosecond);
