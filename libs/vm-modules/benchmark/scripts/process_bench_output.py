...
def print_exec_time_charge_corr(csv_file_path, root):
    ...
    aggregate_stats = os.path.join(
        root, os.path.splitext(csv_file_path)[0]+"_stats")
...
def parse_command_line():
    parser = argparse.ArgumentParser(
        description='Plot GBench output for VM ML operations charge calculation')

    parser.add_argument('csv_file', type=str,
                        help='Location of GBench output csv file')
