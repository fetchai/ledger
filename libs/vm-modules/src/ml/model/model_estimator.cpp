                                           ...
                                           SizeType const &hidden_nodes)
{
  ...
  state_.forward_pass_cost = state_.forward_pass_cost +
                             inputs * hidden_nodes * FORWARD_DENSE_QUAD_COEF;
  ...
  state_.backward_pass_cost =
      state_.backward_pass_cost +
      inputs * hidden_nodes * BACKWARD_DENSE_QUAD_COEF;
                                               ...
                                               Ptr<String> const &optimiser)
{
  ...
  if (!model_.model_->loss_set_)
  {
    if (loss->string() == "mse")
    {
      ...
      state_.forward_pass_cost = state_.forward_pass_cost +
                                 MSE_FORWARD_IMPACT * state_.last_layer_size;
      state_.backward_pass_cost =
          state_.backward_pass_cost +
          MSE_BACKWARD_IMPACT * state_.last_layer_size;
      state_.ops_count++;
      success = true;
    }
    else if (loss->string() == "cel")
    {
      ...
      state_.forward_pass_cost = state_.forward_pass_cost +
                                 CEL_FORWARD_IMPACT * state_.last_layer_size;
      state_.backward_pass_cost =
          state_.backward_pass_cost +
          CEL_BACKWARD_IMPACT * state_.last_layer_size;
      state_.ops_count++;
      success = true;
    }
    else if (loss->string() == "scel")
    {
      ...
      state_.forward_pass_cost =
          state_.forward_pass_cost +
          SCEL_FORWARD_IMPACT * state_.last_layer_size;
      state_.backward_pass_cost =
          state_.backward_pass_cost +
          SCEL_BACKWARD_IMPACT * state_.last_layer_size;
      state_.ops_count++;
      success = true;
    }
    else
                                 ...
                                 SizeType const &batch_size)
{
  ...
  estimate = estimate + PREDICT_BATCH_LAYER_COEF *
                            state_.subset_size * state_.ops_count;
  ...
  estimate = estimate + state_.optimiser_step_impact *
                            number_of_batches * state_.weights_size_sum;
...
{
  DataType estimate =
      DESERIALISATION_PER_CHAR_COEF * model_string->string().size();
