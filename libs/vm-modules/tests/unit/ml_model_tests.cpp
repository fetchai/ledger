...
TEST_F(VMModelTests, model_add_invalid_layers)
{
  ...
  static char const *INVALID_PARAMS_DENSE_RELU = R"(model.add("dense", 1u64, 1u64, 1u64, "relu");)";

  static char const *INVALID_PARAMS_CONV_NOACT = R"(model.add("conv1d", 10u64);)";
  static char const *INVALID_PARAMS_CONV_RELU  = R"(model.add("conv1d", 10u64, 10u64, "relu");)";

  static std::vector<std::string> COMPILABLE_CASES{
      INVALID_ACTIVATION_DENSE,   INVALID_ACTIVATION_CONV,   INVALID_LAYER_TYPE,
      INVALID_PARAMS_DENSE_NOACT, INVALID_PARAMS_DENSE_RELU, INVALID_PARAMS_CONV_NOACT,
      INVALID_PARAMS_CONV_RELU,
  };

  for (auto test_case : COMPILABLE_CASES)
  {
  ...
  static std::vector<std::string> UNCOMPILABLE_CASES{
  ...
  for (auto test_case : UNCOMPILABLE_CASES)
  {
...
TEST_F(VMModelTests, model_compilation_params)
{
  static char const *SOURCE = R"(
      function main()
         ...
         model.add("dense", 10u64, 10u64, "relu");
         model.add("dense", 10u64, 10u64, "relu");
         model.add("conv1d", 10u64, 10u64, 2u64, 2u64, "relu");
         model.add("dense", 10u64, 1u64);
         <<TOKEN>>
      endfunction
    )";

  static char const *MSE_ADAM = R"(model.compile("mse", "adam");)";

  for (auto test_case : {MSE_ADAM})
  {
    std::cout << "Testing valid model compilation params: " << test_case << std::endl;
    std::string const src = std::regex_replace(SOURCE, std::regex("<<TOKEN>>"), test_case);
    ASSERT_TRUE(toolkit.Compile(src));
    EXPECT_TRUE(toolkit.Run());
    // EXPECT_THROW(toolkit.Run(), std::runtime_error);
  ...
  // invalid loss
  // invalid optimizer
  // empty hidden layers or 1 hiddne layer to regressor
  // hidden layers to sequential
}

TEST_F(VMModelTests, model_invalid_serialization)
{
  // TODO: impl!
  // without underlying model
  // uncompiled
  // with no dataloader
  // without optimizer
}
