...
public:
  ...
  {
    DataType from_size   = static_cast<DataType>(fetch::math::Tensor<DataType>::PaddedSizeFromShape(from_shape));
    DataType padded_size = static_cast<DataType>(fetch::math::Tensor<DataType>::PaddedSizeFromShape(new_shape));
...
TEST_F(MathTensorEstimatorTests, tensor_estimator_min_test)
{
  ...
  for (SizeType n_dims = min_dims; n_dims < max_dims; n_dims += dims_step)
  {
         ...
         cur_dim_size += dim_step)
    {
      ...
      DataType padded_size = static_cast<DataType>(fetch::math::Tensor<DataType>::PaddedSizeFromShape(tensor_shape));
      DataType size        = static_cast<DataType>(fetch::math::Tensor<DataType>::SizeFromShape(tensor_shape));
...
TEST_F(MathTensorEstimatorTests, tensor_estimator_transpose_test)
{
       ...
       cur_dim_size += DimSizeStep())
  {
    ...
    DataType padded_from_size = static_cast<DataType>(fetch::math::Tensor<DataType>::PaddedSizeFromShape(tensor_shape));
    DataType padded_to_size   = static_cast<DataType>(fetch::math::Tensor<DataType>::PaddedSizeFromShape(new_shape));
