...
class Muddle2LearnerNetworkerBis : public AbstractLearnerNetworker
{
public:
  ...
  Muddle2LearnerNetworkerBis(const std::string cloud_config,
                          std::size_t instance_number,
                          std::shared_ptr<NetworkManager> netm= std::shared_ptr<NetworkManager>());
  virtual ~Muddle2LearnerNetworkerBis();
...
protected:
  ...
  class Muddle2LearnerNetworkerBisProtocol : public fetch::service::Protocol
  {
  public:
    Muddle2LearnerNetworkerBisProtocol(Muddle2LearnerNetworkerBis &sample);
  };

  std::shared_ptr<NetworkManager> netm;
  MuddlePtr mud;
  std::shared_ptr<Server> server;
  std::shared_ptr<Muddle2LearnerNetworkerBisProtocol> proto;

  mutable Mutex mutex;
  IntermediateList updates;
  Peers peers;
private:
  Muddle2LearnerNetworkerBis(const Muddle2LearnerNetworkerBis &other) = delete;
  Muddle2LearnerNetworkerBis &operator=(const Muddle2LearnerNetworkerBis &other)  = delete;
  bool                    operator==(const Muddle2LearnerNetworkerBis &other) = delete;
  bool                    operator<(const Muddle2LearnerNetworkerBis &other)  = delete;
};
