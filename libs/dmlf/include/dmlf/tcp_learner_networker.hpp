...
class TcpLearnerNetworker : public ILearnerNetworker
{
  ...
  {
  public:
    DmlfTCPServer(uint16_t port, network_manager_type const &network_manager,
                  TcpLearnerNetworker &learner)
      : TCPServer(port, network_manager)
      , learner_{learner}
    {}
    void PushRequest(connection_handle_type client, network::message_type const &msg) override
    {
...
public:
  ...
  TcpLearnerNetworker(std::vector<Uri>                         peers,
                      std::shared_ptr<network::NetworkManager> nnmm = nullptr)
    : TcpLearnerNetworker(ephem_port_(), peers, nnmm)
  {}

  TcpLearnerNetworker(uint16_t port, std::vector<Uri> peers,
                      std::shared_ptr<network::NetworkManager> nnmm = nullptr)
    : nm_{nnmm}
    , nm_mine_{false}
...
protected:
  Intermediate getUpdateIntermediate() override;
...
private:
  ...
  using QueueIntermediate = std::deque<Intermediate>;

  QueueUpdates      updates_;
  QueueIntermediate updates_bytes_;
  ...
  void add_peer_(Uri &uri)
  {
    ...
    conn->OnMessage([this](network::message_type const &upd) { on_new_update_(upd); });
    clients_.push_back(conn);
    std::cout << "Added connection to " << uri.AsPeer().address() << ":" << uri.AsPeer().port()
              << std::endl;
  }

  void on_new_update_(network::message_type const &serialized_update)
  {
    /*
