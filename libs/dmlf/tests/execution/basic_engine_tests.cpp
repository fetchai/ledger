...
{
  ExecutionResult result = RunStatelessTest(
      AddFixed, "add", Params{LedgerVariant(fp64_t::FromFloat(4.5)), LedgerVariant(fp32_t::FromFloat(5.5))});
  ASSERT_TRUE(result.succeeded()) << result.error().message() << '\n';
...
TEST(BasicVmEngineDmlfTests, ArrayArrayOpTests)
{
  ...
  RunArrayTest("doFixed32",
               std::vector<fp32_t>{fp32_t::FromFloat(1.2), fp32_t::FromFloat(2.4), fp32_t::FromFloat(3.7), fp32_t::FromFloat(4.8)});
  RunArrayTest("doFixed64",
               std::vector<fp64_t>{fp64_t::FromFloat(1.3), fp64_t::FromFloat(2.2), fp64_t::FromFloat(3.5), fp64_t::FromFloat(4.7)});
