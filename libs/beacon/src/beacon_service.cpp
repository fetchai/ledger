...
// It is possible to detect that the node is catching up (perhaps after a restart)
// by determining the difference between the entropy it is trying to use and
// the entropy most recently generated by other nodes
bool BeaconService::AreCatchingUpToEntropy(uint64_t block_number)
{
  FETCH_LOCK(mutex_);
