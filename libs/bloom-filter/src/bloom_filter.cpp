...
constexpr std::size_t const INITIAL_SIZE_IN_BITS = 8 * 2 * 1024 * 1024;
    ...
    fetch::byte_array::ConstByteArray const &element) const
{
  ...
  {
    std::size_t const bit_index = hash % bits_.size();
    ++bits_checked;
...
BitVector const &BasicBloomFilter::underlying_bit_vector() const
{
  return bits_;
}
