...
TEST(TypeUtilTests, BasicListInterface) {
	...
	ASSERT_TYPE_EQ(fetch::type_util::HeadArgumentT<std::is_constructible<NonConstructible, char, int, NonConstructible, double>>, NonConstructible);
...
TEST(TypeUtilTests, ScalarFunctionals) {
	ASSERT_TYPE_EQ(fetch::type_util::AccumulateT<fetch::pack::Pack, char, int, NonConstructible, double>,
	...
	ASSERT_UNTRUE(fetch::type_util::AnyV<std::is_constructible, NonConstructible, NonConstructible, NonConstructible>);
	...
	ASSERT_HOLDS(fetch::type_util::SatisfiesV<double, std::is_constructible, std::is_scalar, std::is_arithmetic>);
	ASSERT_UNTRUE(fetch::type_util::SatisfiesV<double, std::is_constructible, std::is_scalar, std::is_arithmetic, std::is_integral>);
}

template<class... Ts> using Ctor = fetch::type_util::Bind<fetch::pack::Pack, char, int>::template type<Ts...>;

inline void f(char, int, NonConstructible, double);
inline int g(char, int, NonConstructible);

TEST(TypeUtilTests, ScalarUtils) {
	ASSERT_TYPE_EQ(Ctor<NonConstructible, double>, fetch::pack::Pack<char, int, NonConstructible, double>);
	...
	ASSERT_TYPE_EQ(fetch::type_util::InvokeResultT<decltype(&f), char, int, NonConstructible, double>, void);
	...
	ASSERT_TYPE_EQ(fetch::type_util::LiftIntegerSequenceT<std::index_sequence<0, 3, 42>>, fetch::pack::Pack<std::index_sequence<0>, std::index_sequence<3>, std::index_sequence<42>>);
...
TEST(TypeUtilTests, SwitchesAndSelects) {
	ASSERT_TYPE_EQ(fetch::type_util::SwitchT<
		       std::false_type, char,
		       std::false_type, int,
...
template<std::size_t index> class Setter: public fetch::pack::SizeConstant<index> {
	Values &values;
public:
	using fetch::pack::SizeConstant<index>::value;

	Setter(Values &values): values(values) {}
...
TEST(TypeUtilTests, BinarySwitch) {
	...
	ASSERT_HOLDS(BS::Call(fetch::pack::SizeConstant<0>{}, [](auto &&x){ x.get() = 'H'; return 0; }, v) == 0);
