...
template<class F> struct StaticOperator {
...
template<template<class...> class F> using StlOperator = StaticOperator<F<void>>;

template<typename... Ts> using Conjunction = LeftAccumulateT<StlOperator<std::logical_and>::template type, std::true_type, Ts...>;
...
template<typename... Ts> using Disjunction = LeftAccumulateT<StlOperator<std::logical_or>::template type, std::false_type, Ts...>;
...
template<class T, class... Ts> struct AreSimilar: Conjunction<AreSimilar<T, Ts>...> {};

template<class T, class... Ts> static constexpr auto AreSimilarV = AreSimilar<T, Ts...>::value;

template<class T1, class T2> struct AreSimilar<T1, T2>: std::is_same<T1, T2> {};

template<template<class...> class F, class... Args1, class... Args2> struct AreSimilar<F<Args1...>, F<Args2...>>: std::true_type {};
...
template<class... Tuples> using ConcatT = typename Concat<Tuples...>::type;

template<class... Ts1, class... Ts2> struct Concat<std::tuple<Ts1...>, std::tuple<Ts2...>>: TypeBox<std::tuple<Ts1..., Ts2...>> {};

template<class Car, class Cdr> using Cons = Concat<std::tuple<Car>, Cdr>;

template<class Car, class Cdr> using ConsT = typename Cons<Car, Cdr>::type;

template<class Init, class Last> using Append = Concat<Init, std::tuple<Last>>;

template<class Init, class Last> using AppendT = typename Append<Init, Last>::type;
