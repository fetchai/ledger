...
template<class T> using TypeConstant = pack::Constant<T>;
template<class T> using TypeConstantT = typename TypeConstant<T>::type;  // technically, an identity mapping

template<template<class...> class F, class... List> using Accumulate = pack::Accumulate<F, pack::ConcatT<List...>>;

template <template <class...> class F, class... List>
using AccumulateT = typename Accumulate<F, List...>::type;

template<template<class...> class F, class... List> static constexpr auto AccumulateV = Accumulate<F, List...>::value;
...
template<class F, class... Args> using IsInvocable = pack::IsInvocable<F, pack::ConcatT<Args...>>;
template<class F, class... Args> static constexpr auto IsInvocableV = IsInvocable<F, Args...>::value;

template<class F, class... Args> using InvokeResult = pack::InvokeResult<F, pack::ConcatT<Args...>>;
...
struct SwitchNode {
	...
	template<class Id> static constexpr bool CanHandle(Id) noexcept(noexcept(LowerBound() <= Id::value)) {
		return LowerBound() <= Id::value;
	}
	template<class Id, class F, class... Args> static constexpr decltype(auto) Call(Id selector, F &&f, Args &&...args)
		noexcept(
			...
			&& noexcept(ReturnZero<decltype(Right::Call(std::declval<Id>(), std::declval<F>(), std::declval<Args>()...))>::Call()))

	{
		...
		return ReturnZero<decltype(Right::Call(selector, std::forward<F>(f), std::forward<Args>(args)...))>::Call();
	}
};

template<class ParticularId> struct SwitchLeaf {
	static constexpr auto LowerBound() noexcept {
		return ParticularId::value;
	}
	template<class Id> static constexpr bool CanHandle(Id)
		noexcept(noexcept(Id::value == LowerBound()))
	{
		return Id::value == LowerBound();
	}
	template<class Id, class F, class... Args> static constexpr decltype(auto) Call(Id, F &&f, Args &&...args)
		noexcept(noexcept(std::declval<F>()(ParticularId(std::declval<Args>())...)))
	{
...
template<class Ids> struct BinarySwitch: SwitchNode<BinarySwitch<pack::LeftHalfT<Ids>>, BinarySwitch<pack::RightHalfT<Ids>>> {};

template<class ParticularId> struct BinarySwitch<pack::Pack<ParticularId>>: SwitchLeaf<ParticularId> {};

} // namespace detail_

template<class... Ids> using BinarySwitch = detail_::BinarySwitch<pack::UniqueSortT<pack::ConcatT<Ids...>>>;

template<class Sequence> struct LiftIntegerSequence;
template<class Sequence> using LiftIntegerSequenceT = typename LiftIntegerSequence<Sequence>::type;
template<class Id, Id... ids> struct LiftIntegerSequence<std::integer_sequence<Id, ids...>>: TypeConstant<pack::Pack<std::integer_sequence<Id, ids>...>> {};
