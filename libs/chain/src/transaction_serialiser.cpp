...
uint8_t ReadSingleByte(serialisers::MsgPackSerialiser &buffer)
{
  uint8_t value{0};
...
void Decode(MsgPackSerialiser &buffer, Address &address)
{
...
template <typename T>
meta::IfIsInteger<T, T> Decode(MsgPackSerialiser &buffer)
{
  return detail::DecodeInteger<T>(buffer);
}

template <typename T>
meta::IfIsInteger<T> Decode(MsgPackSerialiser &buffer, T &value)
{
  value = Decode<T>(buffer);
}

void Decode(MsgPackSerialiser &buffer, BitVector &bits)
{
...
void Decode(MsgPackSerialiser &buffer, ConstByteArray &bytes)
{
...
void Decode(MsgPackSerialiser &buffer, Identity &identity)
{
  // read the identifier
...
void DecodeFixed(MsgPackSerialiser &buffer, uint64_t &value)
{
...
TransactionSerialiser::TransactionSerialiser(ConstByteArray data)
  : serial_data_{std::move(data)}
{}

ByteArray TransactionSerialiser::SerialisePayload(Transaction const &tx)
{
...
bool TransactionSerialiser::Serialise(Transaction const &tx)
{
  // serialise the actual buffer
  auto buffer = SerialisePayload(tx);
...
bool TransactionSerialiser::Deserialise(Transaction &tx) const
{
  serialisers::MsgPackSerialiser buffer{serial_data_};
...
TransactionSerialiser &TransactionSerialiser::operator<<(Transaction const &tx)
{
  if (!Serialise(tx))
  {
    throw std::runtime_error("Unable to serialise transaction from input stream");
  }

  return *this;
}

TransactionSerialiser &TransactionSerialiser::operator>>(Transaction &tx)
{
  if (!Deserialise(tx))
  {
    throw std::runtime_error("Unable to deserialise transaction from input stream");
  }
