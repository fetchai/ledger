...
class OutboundSearchConversationWorkerTask
  : public OutboundConversationWorkerTask
{
public:
  ...
  OutboundSearchConversationWorkerTask(Core &core, const std::string &core_key, const Uri &core_uri,
      const Uri &search_uri, std::shared_ptr<OutboundConversations> outbounds, const ConversationMap& ConversationMap)
...
{
  worker = std::make_shared<OutboundSearchConversationWorkerTask>(search_uri, outbounds, ident2conversation);
