...
public:
  ...
  template <typename T>
  typename IgnoredSerialiser<T, SizeCounter>::DriverType &operator<<(T const & /*unused*/);

  template <typename T>
  typename IgnoredSerialiser<T, SizeCounter>::DriverType &operator>>(T & /*unused*/);

  template <typename T>
  typename ForwardSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename ForwardSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  template <typename T>
  typename IntegerSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename IntegerSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  template <typename T>
  typename FloatSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename FloatSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  template <typename T>
  typename BooleanSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename BooleanSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  template <typename T>
  typename StringSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename StringSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  template <typename T>
  typename BinarySerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename BinarySerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  ArrayConstructor NewArrayConstructor();

  template <typename T>
  typename ArraySerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename ArraySerialiser<T, SizeCounter>::DriverType &operator>>(T &val);

  MapConstructor NewMapConstructor();

  template <typename T>
  typename MapSerialiser<T, SizeCounter>::DriverType &operator<<(T const &val);

  template <typename T>
  typename MapSerialiser<T, SizeCounter>::DriverType &operator>>(T &val);
...
template <typename T>
typename IgnoredSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(
    T const & /*unused*/)
{
  return *this;
}
...
template <typename T>
typename IgnoredSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T & /*unused*/)
{
  return *this;
}

template <typename T>
typename ForwardSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  try
  {
    ForwardSerialiser<T, SizeCounter>::Serialise(*this, val);
  }
...
template <typename T>
typename ForwardSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  try
  {
    ForwardSerialiser<T, SizeCounter>::Deserialise(*this, val);
  }
  catch (std::exception const &e)
  {
...
template <typename T>
typename IntegerSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  try
  {
    IntegerSerialiser<T, SizeCounter>::Serialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error serializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename IntegerSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  try
  {
    IntegerSerialiser<T, SizeCounter>::Deserialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error deserializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename FloatSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  try
  {
    FloatSerialiser<T, SizeCounter>::Serialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error serializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename FloatSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  try
  {
    FloatSerialiser<T, SizeCounter>::Deserialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error deserializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename BooleanSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  try
  {
    BooleanSerialiser<T, SizeCounter>::Serialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error serializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename BooleanSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  try
  {
    BooleanSerialiser<T, SizeCounter>::Deserialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error deserializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename StringSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  try
  {
    StringSerialiser<T, SizeCounter>::Serialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error serializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename StringSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  try
  {
    StringSerialiser<T, SizeCounter>::Deserialise(*this, val);
  }
  catch (std::exception const &e)
  {
    throw std::runtime_error("Error deserializing " + static_cast<std::string>(typeid(T).name()) +
...
template <typename T>
typename BinarySerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  using Serialiser = BinarySerialiser<T, SizeCounter>;
  using Constructor =
...
template <typename T>
typename BinarySerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  using Serialiser = BinarySerialiser<T, SizeCounter>;
  try
  {
    interfaces::BinaryDeserialiser<SizeCounter> stream(*this);
...
template <typename T>
typename ArraySerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  using Serialiser = ArraySerialiser<T, SizeCounter>;
  try
  {
...
template <typename T>
typename ArraySerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  using Serialiser        = ArraySerialiser<T, SizeCounter>;
  using ArrayDeserialiser = interfaces::ArrayDeserialiser<SizeCounter>;
...
template <typename T>
typename MapSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator<<(T const &val)
{
  using Serialiser = MapSerialiser<T, SizeCounter>;
...
template <typename T>
typename MapSerialiser<T, SizeCounter>::DriverType &SizeCounter::operator>>(T &val)
{
  using Serialiser      = MapSerialiser<T, SizeCounter>;
  using MapDeserialiser = interfaces::MapDeserialiser<SizeCounter>;
 ...
 * environment.
 *
 * @tparam T Represents type with STREAM/BUFFER like API (e.g. SizeCounter, MsgPackSerialiser,
 * MsgPackSerialiser, etc. ...), with clear preference to use here size count
