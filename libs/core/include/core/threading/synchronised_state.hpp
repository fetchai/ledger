                                   ...
                                   std::chrono::duration<R, P> const &max_wait_time) const
{
  ...
  return condition_.wait_for(lock, max_wait_time, [this, &state]() {
    return state_ == state;
  });
}
...
{
  UniqueLock lock{lock_};
  condition_.wait(lock, [this, &state]() {
    return state_ == state;
  });
}
...
{
  UniqueLock lock{lock_};
  condition_.wait(lock, [this, h = std::forward<Handler>(handler)]() {
    return h(state_);
  });
}
...
template<typename S>
template<typename Handler, typename R, typename P>
bool SynchronisedState<S>::WaitFor(std::chrono::duration<R,P> const &max_wait_time, Handler &&handler) const
{
  UniqueLock lock{lock_};
  return condition_.wait_for(lock, max_wait_time, [this, h = std::forward<Handler>(handler)]() {
    return h(state_);
  });
}
