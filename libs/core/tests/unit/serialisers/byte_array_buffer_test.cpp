...
public:
  ...
  static void Serialise(Constructor &array_constructor, Type const &a)
  {
  ...
  static void Deserialise(ArrayDeserialiser &array, Type &a)
  {
...
template <typename D>
struct ArraySerialiser<A<void>, D>
{
public:
  using Type       = A<void>;
  using DriverType = D;
...
class MsgPackSerialiserTest : public testing::Test
{
protected:
  using B = A<A<A<void>>>;

  void test_nested_append_serialisation(MsgPackSerialiser &stream)
  {
    B const            b0{"b0x", "b0y"};
    B const            b1{"b1x", "b1y"};
...
TEST_F(MsgPackSerialiserTest, test_seek_position_is_zero_after_stream_construction)
{
  MsgPackSerialiser stream;
  EXPECT_EQ(0, stream.tell());
}

TEST_F(MsgPackSerialiserTest, test_basic_allocate_size)
{
...
TEST_F(MsgPackSerialiserTest, test_allocate_with_offset)
{
...
TEST_F(MsgPackSerialiserTest, verify_correctness_of_copy_and_comparison_behaviour_of_B_type)
{
  B const b0{"b0", "b0"};
  B       b0_copy{b0};
...
TEST_F(MsgPackSerialiserTest, test_basic)
{
  MsgPackSerialiser stream;
  test_nested_append_serialisation(stream);
}

TEST_F(MsgPackSerialiserTest, test_stream_with_preexisting_offset)
{
...
TEST_F(MsgPackSerialiserTest, test_stream_relative_resize_with_preexisting_offset)
{
  constexpr std::size_t preallocated_amount = 100;
  MsgPackSerialiser     stream;
...
TEST_F(MsgPackSerialiserTest, test_that_default_resize_paradigm_is_relative)
{
  constexpr std::size_t delta_size = 10;
  // Setup
  MsgPackSerialiser stream;
...
TEST_F(MsgPackSerialiserTest, test_stream_absolute_resize_with_preexisting_offset)
{
