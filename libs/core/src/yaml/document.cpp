    ...
    switch (token.type)
    {
        ...
        case STRING_MULTILINE:
            ...
            while(pos<=token.second) {
                prev_pos = pos;
                while(pos<=token.second && *(document.char_pointer() + pos)!=0x0A && *(document.char_pointer() + pos)!=0x0D && *(document.char_pointer() + pos)!='\\')
                    ++pos;
                ...
                if(c==0x0A || c==0x0D) {  
                    while(pos<=token.second && (*(document.char_pointer() + pos)==0x0D || *(document.char_pointer() + pos)==0x0A))
                      ++pos;
...
void YamlDocument::Parse(ConstByteArray const &document)
{
    ...
    for (std::size_t idx = 0, end = tokens_.size(); idx < end; ++idx)
    {
        ...
        bool const is_primitive = (token.type == KEYWORD_TRUE) || (token.type == KEYWORD_FALSE) ||
                              (token.type == KEYWORD_NULL) || (token.type == STRING) || (token.type == STRING_MULTILINE) ||
                              (token.type == NUMBER_INT) || (token.type == NUMBER_FLOAT) || (token.type == NUMBER_HEX) || (token.type == NUMBER_OCT) ||
                              (token.type == KEYWORD_INF) || (token.type == KEYWORD_NEG_INF) || (token.type == KEYWORD_NAN);
        ...
        {    	
            ...
            {
                throw YamlParseException("Invalid parser state: value mode, but previous element in stack is not array and not object");
            }
            ...
            if(current == nullptr) {
                ...
                throw YamlParseException("Invalid parser state: reference detected but no object or array in stack!");
            }
        ...
        else if (token.type == KEY)
        {
            ...
            if (context == nullptr || (token.ident > context->ident && token.line != context->line)) // There were no objects in stack
            {
                // define the initial object and add it to the stack
                if(variant_stack.empty()) {
                    variant_ = Variant::Object();                    
            ...
            } else { // We have an object in stack
                ...
                else if (context->data->IsArray()) // add array element
                {
                    ...
                    if(context->line == token.line) {
                        ident = context->ident + 2; // compacted sequence. May be, better calculate spacing between, but actually not necessary
                    }
        ...
        {          
            ...
            if (current== nullptr)
            {
                if(token.type == NEW_ENTRY)
                    ...
                    if(variant_stack.empty()) {
                        variant_ = Variant::Array(0);
                        variant_stack.push_back({&variant_, token.ident, token.line});
                    } else { //TODO: either a bug, or need to get previous object and add value
                        throw YamlParseException("Invalid parser state");
                        //Variant next = Variant::Array(0);
                        //variant_stack.push_back({&next, token.ident, token.line});
                    }
            ...
            } else if(current->data->IsObject() && token.type == NEW_ENTRY) { // Single-line mapping, like: {key:value, key:value}
                state = ObjectState::KEY;
            } else if(!current->data->IsArray()) {
                throw YamlParseException("Invalid parser state");
            }
...
/*                case 0x6C6C756E:  // null
        ...
        switch (c)
        {
            ...
            case '|':
                ...
                while (pos < document.size()) // detecting initial identation  && ((*(ptr + pos)==0x10) || (*(ptr + pos)==0x13) || (*(ptr + pos)==0x20))
                {
                    char const &c = *(ptr + pos);
                ...
                tokens_.push_back({fixed_start, pos, folded ? STRING : STRING_MULTILINE, ident, line});
            ...
            default:  // If none of the above then it is a key
                ...
                if(
                    tokens_.size() && 
                    (
                       ((lastType == NUMBER_INT || lastType == NUMBER_FLOAT || lastType == NUMBER_HEX || lastType == NUMBER_OCT) && lastLine==line) ||
                    ...
                    if(lastType == NUMBER_HEX) {
                        ...
                        if(words16[0]==0x2B65 || words16[0]==0x2D65) { //e+,e-
                            tokens_.back().second = ++pos;
