...
public:
  ...
  {
    filename_ = filename;
    file_handle_ = std::fstream(filename_, std::fstream::in | std::fstream::out | std::fstream::binary);
    if(file_handle_.is_open() )
    {
      Clear();
    }
  ...
  void Get(std::size_t const &i, type &object) 
  {
    ...
    std::size_t pointer_offset = i - mapped_index;
  ...
  {
      //TODO if something is not mapped at specified index 
    ...
    else
    {
      ...
      memcpy(reinterpret_cast<type*>(temp_data_map.data()) , &obj_i , sizeof(type));//writing value to i index
      Set(i,obj_j);
      temp_data_map.unmap();
    }
  }
...
/**
  ...
  {
      return (i >= mapped_index && i < (mapped_index + MAX));
  }
  //////////////////////////////
  void MapIndex(std::size_t const& i)
  {
    mapped_data_.unmap();
    mapped_index = i - (i % MAX);
  ...
  void ResizeFile()
  {
    ...
    char byteValue = '\0';
    int64_t pos = long(header_->size() + (sizeof(type)*(MAX+mapped_index)));
