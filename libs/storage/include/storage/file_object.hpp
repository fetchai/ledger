...
//    , length_(HEADER_SIZE)
//  {
//    // Safe guard - check the stack has been initialized with the 'free' structure
//    if(stack_.size() == 0)
//    {
//      InitaliseStack();
//    }
...
//  }
//
//  void Write(uint8_t const *bytes, uint64_t const &size)
//  {
//    uint64_t total_bytes = size + (byte_index_ + block_number_ * block_type::BYTES);
//
//    uint64_t last_block = CeilDivide(total_bytes, block_type::BYTES);
//
//    --last_block;
//
//    uint64_t first_bytes = block_type::BYTES - byte_index_;
...
//    }
//
//    // Bytes up and including the prev block
//    uint64_t last_bytes = total_bytes - (block_type::BYTES * last_block);
//
//    // Writing first
//    block_type block;
//    stack_.Get(block_index_, block);
...
//    uint64_t n = m + byte_index_ + block_number_ * block_type::BYTES;
//
//    uint64_t last_block = CeilDivide(n, block_type::BYTES);
//
//    --last_block;
//
//    uint64_t first_bytes = block_type::BYTES - byte_index_;
