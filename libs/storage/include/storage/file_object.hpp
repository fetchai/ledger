...
void FileObject<S>::Resize(uint64_t size)
{
  ...
  uint64_t target_blocks = platform::DivideCeil<uint64_t>(size, BlockType::CAPACITY);
...
bool FileObject<S>::VerifyConsistency(std::vector<uint64_t> const &ids)
{
  ...
  for (auto const &id : ids)
  {
    ...
    uint64_t expected_blocks = platform::DivideCeil<uint64_t>(file_bytes, BlockType::CAPACITY);
