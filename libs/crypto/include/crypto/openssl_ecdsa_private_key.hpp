...
private:
   ...
   ECDSAPrivateKey(
        ...
        )
    {
        ...
        {
            throw std::runtime_error("ECDSAPrivateKey::Convert2BIGNUM(const byte_array::ConstByteArray&): Lenght of provided byte array does not correspond to expected lenght for selected elliptic curve");
        }

        uniq_ptr_type<BIGNUM, del_strat_type::clearing> private_key_as_BN(BN_new());
        BN_bin2bn(key_data.pointer(), static_cast<int>(ecdsa_curve_type::privateKeySize), private_key_as_BN.get());

        if (!private_key_as_BN)
        {
            throw std::runtime_error("ECDSAPrivateKey::Convert2BIGNUM(const byte_array::ConstByteArray&): BN_bin2bn(...) failed.");
        }
    ...
    static public_key_type DerivePublicKey(
        ...
        )
    {
        ...
        if (!public_key)
        {
            throw std::runtime_error("ECDSAPrivateKey::DerivePublicKey(...): EC_POINT_(new/dup)(...) failed.");
        }
    ...
    static byte_array::ByteArray Convert2DER (EC_KEY * key)
    {
        ...
        if (est_size < 1)
        {
            throw std::runtime_error("ECDSAPrivateKey::Convert2DER(...): i2d_ECPrivateKey(..., nullptr) failed.");
        }
    ...
    static ECDSAPrivateKey ConvertFromDER (byte_array::ConstByteArray const & key_data)
    {
        ...
        {
            throw std::runtime_error("ECDSAPrivateKey::ConvertFromDER(...): d2i_ECPrivateKey(...) failed.");
        }
