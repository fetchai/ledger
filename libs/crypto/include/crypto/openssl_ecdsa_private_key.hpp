...
private:
  ...
  static uniq_ptr_type<BIGNUM, del_strat_type::clearing> GetPrivateKeyAsBIGNUM(
      const byte_array::ConstByteArray &key_data)
  {
    if (ecdsa_curve_type::privateKeySize != key_data.size())
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GetPrivateKeyAsBIGNUM(const "
          "byte_array::ConstByteArray&): Lenght of provided byte array does "
          "not correspond to expected lenght for selected elliptic curve");
    }
    uniq_ptr_type<BIGNUM, del_strat_type::clearing> private_key_as_BN(BN_new());
    BN_bin2bn(key_data.pointer(), int(ecdsa_curve_type::privateKeySize),
    ...
    if (!private_key_as_BN)
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GetPrivateKeyAsBIGNUM(const "
          "byte_array::ConstByteArray&): BN_bin2bn(...) failed.");
    }
  ...
  static uniq_ptr_type<BIGNUM, del_strat_type::clearing> GetPrivateKeyAsBIGNUM(
      const std::string &hex_string_key_data)
  {
    if (ecdsa_curve_type::privateKeySize * 2 != hex_string_key_data.size())
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GetPrivateKeyAsBIGNUM(const std::string&): Lenght "
          "of provided byte array does not correspond to expected lenght for "
          "priv key for selected elliptic curve");
    }
    ...
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GetPrivateKeyAsBIGNUM(const std::string&): "
          "BN_hex2bn(...) failed.");
    }
    return retval;
  }

  static uniq_ptr_type<EC_KEY> ConvertPrivateKeyBN2ECKEY(
      const BIGNUM *private_key_as_BN)
  {
    uniq_ptr_type<EC_KEY> priv_key{
        EC_KEY_new_by_curve_name(ecdsa_curve_type::nid)};
  ...
  static public_key_type DerivePublicKey(const BIGNUM *private_key_as_BN,
                                         EC_KEY *      private_key)
  {
    ...
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::DerivePublicKey(...): EC_POINT_mul(...) failed.");
    }

    if (!EC_KEY_set_public_key(private_key, public_key.get()))
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::DerivePublicKey(...): EC_KEY_set_public_key(...) "
          "failed.");
    }
  ...
  static public_key_type ExtractPublicKey(const EC_KEY *private_key)
  {
    ...
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::ExtractPublicKey(...): EC_POINT_dup(...) failed.");
    }
  ...
  {
    uniq_ptr_type<EC_KEY> key_pair{
        EC_KEY_new_by_curve_name(ecdsa_curve_type::nid)};
    if (!key_pair)
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GenerateKeyPair(): EC_KEY_new_by_curve_name(...) "
          "failed.");
    }
    ...
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::GenerateKeyPair(): EC_KEY_generate_key(...) "
          "failed.");
    }
  ...
  ECDSAPrivateKey(
      shrd_ptr_type<BIGNUM, del_strat_type::clearing> private_key_as_BN)
      : private_key_(ConvertPrivateKeyBN2ECKEY(private_key_as_BN.get()))
      , public_key_(
            DerivePublicKey(private_key_as_BN.get(), private_key_.get()))
  {}
...
public:
  ...
  ECDSAPrivateKey(const std::string &hex_string_key_data)
      : ECDSAPrivateKey(
            ECDSAPrivateKey::GetPrivateKeyAsBIGNUM(hex_string_key_data))
  {}
  ...
  byte_array::ByteArray KeyAsBin() const
  {
    ...
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::keyAsBin(): EC_KEY_get0_private_key(...) failed.");
    }

    byte_array::ByteArray key_as_bin;
    key_as_bin.Resize(static_cast<std::size_t>(BN_num_bytes(key_as_BN)));

    if (!BN_bn2bin(key_as_BN,
                   static_cast<unsigned char *>(key_as_bin.pointer())))
    {
      throw std::runtime_error(
          "ECDSAPrivateKey::keyAsBin(...): BN_bn2bin(...) failed.");
    }
