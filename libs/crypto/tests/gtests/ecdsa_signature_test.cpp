...
TEST_F(ECDCSASignatureTest, test_moving_semantics_constructor) {
    //* Production code:
    const openssl::ECDSAPrivateKey<> priv_key(priv_key_data);

    ECDSASignature<eECDSABinaryDataFormat::canonical> signature0 {
        ECDSASignature<eECDSABinaryDataFormat::canonical>::Sign(priv_key, test_data)};
...
TEST_F(ECDCSASignatureTest, test_copy_constructor) {
    ...
    ECDSASignature<eECDSABinaryDataFormat::DER> signature1 {signature0};

    EXPECT_TRUE(signature0.signature_ECDSA_SIG());
}
