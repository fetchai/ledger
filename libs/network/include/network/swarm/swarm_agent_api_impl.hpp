  ...
  uint32_t idlespeed_;
    ...
    void DoIdle()
    {
        ...
        if (this->onIdle_)
        {
            ...
            {
                std::cerr
                    << "SwarmAgentApiImpl::DoIdle Exception ignored:"
                    << x.what()
                    << std::endl;
            }
            catch(...)
    ...
    virtual void ToPing(
        std::function<
            void (SwarmAgentApi &api, const std::string &host)
        > action)
    {
        toPing_ = action;
    }

    virtual void OnPingSucceeded(
    ...
    virtual void ToDiscoverPeers       (
        std::function<
            void (SwarmAgentApi &api, const std::string &host, uint32_t count)
        > action)
    {
    ...
    virtual void OnNewBlockIdFound(
        std::function<
            void (const std::string &host, const std::string &blockid)
        > cb)
    {
    ...
    virtual void DoBlockIdRepeated(
        const std::string &host, const std::string &blockid)
    {
        threadingSystem_ -> Post([this, host, blockid]{
    ...
    virtual void OnNewTxnListIdFound(
        std::function<
            void (const std::string &host, const std::string &txnlistid)
        > cb)
    {
        onNewTxnListIdFound_ = cb;
    }

    virtual void DoGetTxnList(
        const std::string &host, const std::string &txnlistid)
    {
    }
    ...
    virtual void ToGetKarma(
        std::function<double (const std::string &host)> query)
    {
    ...
    virtual void ToGetPeers(
        std::function<
            std::list<std::string>(uint32_t count, double minKarma)
        > query)
    {
    ...
    virtual void ToAddKarmaMax(
        std::function<
            void(const std::string &host, double karma, double limit)
        > action)
    {
        this->toAddKarmaMax_ = action;
    }
    virtual void AddKarmaMax(
        const std::string &host, double karma, double limit)
    {
