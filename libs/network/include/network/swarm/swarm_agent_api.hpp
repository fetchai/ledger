...
public:
  ...
  virtual void OnPingSucceeded(
      std::function<void(const std::string &host)> cb) = 0;
  virtual void OnPingFailed(
      std::function<void(const std::string &host)> cb) = 0;
  ...
  virtual void DoTransactionListBuilt(
      const std::list<std::string> &txnlist) = 0;
  ...
  virtual void OnBlockIdRepeated(
      std::function<void(const std::string &host, const std::string &blockid)>
          cb)                                         = 0;
  virtual void DoGetBlock(const std::string &host,
  ...
  virtual void   AddKarmaMax(const std::string &host, double karma,
                             double limit)                                 = 0;
