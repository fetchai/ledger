...
public:
  AuthenticationProtocol(AuthenticationLogic<D> *auth_logic) : Protocol()
  {
    ...
    this->ExposeWithClientArg(HELLO, auth_logic,
                              &AuthenticationLogic<D>::Hello);
    this->ExposeWithClientArg(GET_CHALLENGE, auth_logic,
                              &AuthenticationLogic<D>::GetChallenge);
...
public:
      ...
      : ServiceServer(port, tm)
  {
    ...
    auth_proto_.reset(
        new AuthenticationProtocol<fetch::NodeDetails>(auth_logic_.get()));

    test_proto_.AddMiddleware(
        [this](network::AbstractConnection::connection_handle_type const &n,
          ...
          std::cout << "Is authenticated? " << (authed ? "YES" : "NO")
                    << std::endl;
          ...
          if (!authed)
          {
            throw serializers::SerializableException(
                0, byte_array_type(
                       "Please authenticate by using the AUTH protocol."));
          }
        });
