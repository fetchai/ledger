...
Adapter::adapter_list_type Adapter::GetAdapters()
{
  ...
  if (::getifaddrs(&iflist_start))
  {
    throw std::runtime_error(
        "Unable to query interface information from operating system");
  }
  ...
  for (struct ifaddrs *curr = iflist_start; curr; curr = curr->ifa_next)
  {
    ...
    bool const is_inet =
        (curr->ifa_addr && curr->ifa_addr->sa_family == AF_INET);
    if (!is_inet) continue;

    auto *inet_addr = reinterpret_cast<struct sockaddr_in *>(curr->ifa_addr);
    auto *net_mask_addr =
        reinterpret_cast<struct sockaddr_in *>(curr->ifa_netmask);
