...
inline typename memory::VectorSlice<T,S>::type Pearson(memory::VectorSlice<T,S> const &a, memory::VectorSlice<T,S> const &b)   
{
  ...
  type meanA = a.in_parallel().Reduce([](vector_register_type const &x,
      vector_register_type const &y) {
      return x + y;
    });
  
  type meanB = b.in_parallel().Reduce([](vector_register_type const &x,
      vector_register_type const &y) {
      return x + y;
    });  
  ...
  type innerA = a.in_parallel().SumReduce(memory::TrivialRange(0, a.size()), [mA](vector_register_type const &x) {
      vector_register_type d = x - mA;
      return d * d;
    });  

  
  type innerB = b.in_parallel().SumReduce(memory::TrivialRange(0, b.size()), [mB](vector_register_type const &x) {
      vector_register_type d = x - mB;
      return d * d;
    });

  type top = a.in_parallel().SumReduce(memory::TrivialRange(0, a.size()),[mA, mB](vector_register_type const &x, vector_register_type const &y) {
