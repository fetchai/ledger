...
inline typename memory::VectorSlice<T,S>::type Braycurtis(memory::VectorSlice<T,S> const &a, memory::VectorSlice<T,S> const &b)   
{
  ...
  type sumB = a.in_parallel().SumReduce(memory::TrivialRange(0, b.size()), [](vector_register_type const &x, vector_register_type const &y) {
      return abs(x+y);
    }, b);
  ...
  inline typename ShapeLessArray<T,C>::type Braycurtis( ShapeLessArray<T,C> const &a,  ShapeLessArray<T,C> const &b) 
{
