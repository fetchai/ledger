...
inline typename memory::VectorSlice<T,S>::type Hamming(memory::VectorSlice<T,S> const &a, memory::VectorSlice<T,S> const &b) 
{
  ...
  double dist = a.in_parallel().SumReduce(memory::TrivialRange(0, a.size()), [](vector_register_type const &x,  vector_register_type const &y) {
      return x != y;
    }, b);
  ...
  inline typename ShapeLessArray<T,C>::type Hamming( ShapeLessArray<T,C> const &a,  ShapeLessArray<T,C> const &b) 
{
