                  ...
                  double const &beta, Matrix<double> &c)
  {
    ...
    for (j = 0; j < c.width(); ++j)
    {
      ...
      if (beta == 0.0)
      {
        ...
        auto ret_slice = c.data().slice(c.padded_height() * j, c.height());
        ret_slice.in_parallel().Apply([vec_zero](vector_register_type &vw_c) { vw_c = vec_zero; });
      }
      else if (beta != 1.0)
      {
      ...
      for (l = 0; l < a.width(); ++l)
      {
        ...
        ret_slice.in_parallel().Apply(
            [vec_temp](vector_register_type const &vr_c, vector_register_type const &vr_a,
                       vector_register_type &vw_c) { vw_c = vr_c + vec_temp * vr_a; },
            slice_c, slice_a);
      }
    }
    return;
