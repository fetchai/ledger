...
class ExpressionNode : public std::enable_shared_from_this<ExpressionNode>
{
 public:
  ...
  // During parsing val is assigned as either a leaf node (in which case VAL will be a number), otherwise it will be
  // an operation to apply on child nodes where it will be stored as a char
  // During Run the graph will be computed and VAL will be replaced by the result of the operations (as a number)
  union VAL
  {
...
class ComputationGraph
{
 private:
  ...
  /**
   * checks whether the new token type is different from the previous token type and thus should count as a new token
   * @return
   */
...
//  using EXPRESSION_STACK = std::stack<std::shared_ptr<expression_node_type>, expression_container_type>;
  ...
  void Tokenize(std::string &input, std::vector<std::string> &ret, std::vector<TOKEN_TYPE> &token_types)
  {
    ...
    for (std::size_t j = 0; j < input.size(); ++j)
    {
      ...
      if (isalpha(input[j])) { cur_type = TOKEN_TYPE::ALPHA; }
      else if (isdigit(input[j])) { cur_type = TOKEN_TYPE::NUMERIC; }
      else if (input[j] == '(') { cur_type = TOKEN_TYPE::OPEN_PAREN; }
      else if (input[j] == ')') { cur_type = TOKEN_TYPE::CLOSE_PAREN; }
      else if (input[j] == '.') { cur_type = TOKEN_TYPE::NUMERIC; }
      ...
      else if (isspace(input[j])) { cur_type = TOKEN_TYPE::IGNORE_TYPE; }
      else
      {
    ...
    for (std::size_t i = 0; i < tokens.size(); ++i)
    {
      ...
      switch (token_types[i])
      {
        ...
        case TOKEN_TYPE::ALPHA:
          break;
//        expression_graph.emplace_back(std::make_unique<ExpressionNode>(double(cur_token - '0')));
...
//        expression_graph.emplace_back(std::make_unique<ExpressionNode>(double(cur_token - '0')));
        ...
        case TOKEN_TYPE::OPERATOR:
          ...
          while (!(operator_stack.empty()) && (helper_funcs::GetPrecedence(operator_stack.top()) >= helper_funcs::GetPrecedence(tokens[i][0])))
          {
