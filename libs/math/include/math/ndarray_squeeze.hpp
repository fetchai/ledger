...
bool ShapeFromSqueeze(std::vector<std::size_t> const &a, std::vector<std::size_t> &b, uint64_t const &axis = uint64_t(-1))
{
  std::size_t i = 0;
  b.clear();
  if(axis == uint64_t(-1))
...
bool ShapeFromSqueeze(std::vector<std::size_t> const &a, std::vector<std::size_t> &b, std::unordered_set< uint64_t > const &axes)
{
  std::size_t i = 0;
  b.clear();
  for(auto const &v : a) {
...
void Reduce(F fnc, NDArray< T, C > &input, NDArray< T, C > &output, 
            T const &def_value = 0, uint64_t const &axis = 0)
{
  std::size_t N;
