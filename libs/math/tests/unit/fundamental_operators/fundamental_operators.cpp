...
TYPED_TEST(FundamentalOperatorsUIntTypeTest, AdditionUIntTest)
{
  ...
  for (std::size_t i{0}; i < 100; ++i)
  {
    ...
    a = TypeParam(gen.generator.AsFP64() * double(max_val));
    b = TypeParam(gen.generator.AsFP64() * double(max_val));
...
TYPED_TEST(FundamentalOperatorsIntTypeTest, AdditionIntTest)
{
  ...
  for (int i{0}; i < 100; ++i)
  {
    ...
    a = TypeParam((gen.generator.AsFP64() * double(max_val)) - (double(max_val) / 2.0));
    b = TypeParam((gen.generator.AsFP64() * double(max_val)) - (double(max_val) / 2.0));
...
TYPED_TEST(FundamentalOperatorsNonIntTypeTest, AdditionNonIntTest)
{
  ...
  // test a few small fixed values
  a = fetch::math::Type<TypeParam>("2.156");
  b = fetch::math::Type<TypeParam>("7.421");
  fetch::math::Add(a, b, ret);
  EXPECT_NEAR(double(ret), 9.577, 1e-9);
  EXPECT_NEAR(double(fetch::math::Add(a, b)), 9.577, 1e-9);

  a = fetch::math::Type<TypeParam>("123.456");
  b = fetch::math::Type<TypeParam>("321.123");
  fetch::math::Add(a, b, ret);
  ...
  for (int i{-100}; i < 100; ++i)
  {
    ...
    fetch::math::Add(a, b, ret);
    EXPECT_EQ(ret, TypeParam{100});
    EXPECT_EQ(fetch::math::Add(a, b), TypeParam{100});
  }
...
TYPED_TEST(FundamentalOperatorsUIntTypeTest, SubtractionUIntTest)
{
  ...
  EXPECT_EQ(ret, max_val - TypeParam(4));
  EXPECT_EQ(fetch::math::Subtract(a, b), max_val - TypeParam(4));
...
TYPED_TEST(FundamentalOperatorsNonIntTypeTest, SubtractionNonIntTest)
{
  ...
  for (int i{-100}; i < 100; ++i)
  {
    ...
    fetch::math::Subtract(b, a, ret);
    EXPECT_EQ(ret, TypeParam{100});
    EXPECT_EQ(fetch::math::Subtract(b, a), TypeParam{100});
  }
...
TYPED_TEST(FundamentalOperatorsNonIntTypeTest, MultiplicationNonIntTest)
{
  ...
  for (int i{-100}; i < 100; ++i)
  {
    ...
    fetch::math::Multiply(b, a, ret);
    EXPECT_EQ(ret, TypeParam(100 * i));
    EXPECT_EQ(fetch::math::Multiply(b, a), TypeParam(100 * i));
  }
