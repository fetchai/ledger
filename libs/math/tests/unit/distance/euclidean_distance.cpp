...
TYPED_TEST(EuclideanTest, simple_test)
{
  ...
  EXPECT_NEAR(double(distance::Euclidean(A, A)), 0, (double)function_tolerance<DataType>());
  ...
  EXPECT_NEAR(double(distance::Euclidean(A, B)), 2, (double)function_tolerance<DataType>());
}
...
TYPED_TEST(EuclideanTest, matrix_euclidean_test)
{
  ...
  ArrayType ret = distance::EuclideanMatrix(A, B, 0);
  ...
  ret = distance::EuclideanMatrix(A, B, 1);
