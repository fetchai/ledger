...
TYPED_TEST(SoftmaxTest, multi_dimension_test)
{
  ...
  // test correct values
  ASSERT_TRUE(
      test_axis0.AllClose(gt_axis0, fetch::math::function_tolerance<typename TypeParam::Type>()) || test_axis0.AllClose(gt_axis0, static_cast<typename TypeParam::Type>(1e-5)));
	ASSERT_TRUE(
	 test_axis1.AllClose(gt_axis1, fetch::math::function_tolerance<typename TypeParam::Type>()) ||  test_axis0.AllClose(gt_axis0, static_cast<typename TypeParam::Type>(1e-5)));
}
