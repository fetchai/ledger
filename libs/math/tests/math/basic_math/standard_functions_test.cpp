...
TYPED_TEST(ClampTest, clamp_array_1D_test)
{
  ...
  ASSERT_TRUE(A_norm.AllClose(A_clamp_expected, DataType{1e-5f}, DataType{1e-5f}));
}
...
TYPED_TEST(ClampTest, clamp_array_inplace_1D_test)
{
  using DataType  = typename TypeParam::Type;
  using ArrayType = TypeParam;
...
TYPED_TEST(ClampTest, clamp_array_inplace_2D_test)
{
  using DataType  = typename TypeParam::Type;
  using ArrayType = TypeParam;
