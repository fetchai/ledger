...
ShapeLessArray<data_type, container_type> RandomArrayZero(std::size_t n, std::size_t m)
{
  static fetch::random::LinearCongruentialGenerator gen;
  ShapeLessArray<data_type, container_type> a1(n);
  for (std::size_t i = 0; i < n; ++i)
...
TEST(ndarray, zeros_out) {
  ...
  std::size_t n = 1000;
  ShapeLessArray<data_type, container_type> test_array = RandomArrayZero(n, n);
