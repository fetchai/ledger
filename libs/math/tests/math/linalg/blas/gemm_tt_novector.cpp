...
TEST(blas_DGEMM, blas_gemm_tt_novector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_tt_novector;
	// Compuing _C <= _alpha * T(_A) * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.6662050112833283);
  Type beta = Type(0.041808725635822386);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > ref_tensor_C = Tensor< Type >::FromString(R"(
  0.33753487777253616 0.07999319684638667 0.26327002186881693;
 0.6124016890409902 0.04498713241227904 0.5664711056756979;
 0.421219482107535 0.08595371501690476 0.3325915901639233
  )");
  TensorView< Type > refC = ref_tensor_C.View();
