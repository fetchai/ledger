...
TEST(blas_gemv, blas_gemv_n_novector1)
{
  ...
  Blas<double, Matrix<double, fetch::memory::SharedArray<double>, fetch::math::RectangularArray<double, fetch::memory::SharedArray<double>, true, false>>, Signature(_y <= _alpha, _A, _x, _n, _beta, _y, _m),
...
TEST(blas_gemv, blas_gemv_n_novector3)
{
  ...
  // Compuing _y = _alpha * _A * _x + _beta * _y
  using type = double;
  type alpha = type(0.7921446012894083);
  type beta  = type(0.7220821201320207);
  int  n     = 1;
  int  m     = 1;
 ...
 0.6075448519014384 0.17052412368729153 0.06505159298527952 0.9488855372533332
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  1.1229442830573635 0.9464022560914767 0.7511555997338858
  )");
...
TEST(blas_gemv, blas_gemv_n_novector5)
{
  ...
  // Compuing _y = _alpha * _A * _x + _beta * _y
  using type = double;
  type alpha = type(0.014235337664599834);
  type beta  = type(0.36476202434181604);
  int  n     = 2;
  int  m     = 3;
 ...
 0.3584657285442726 0.11586905952512971 0.8631034258755935 0.6232981268275579
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  0.05849720439030212 0.713244787222995 0.7607850486168974 0.22023521706348104 0.770967179954561 0.49379559636439074 0.20971103363382473 0.42754101835854963 0.02541912674409519
  )");
...
TEST(blas_gemv, blas_gemv_n_novector6)
{
  ...
  // Compuing _y = _alpha * _A * _x + _beta * _y
  using type = double;
  type alpha = type(0.8181311998352544);
  type beta  = type(0.39446023384552975);
  int  n     = -2;
  int  m     = -3;
 ...
 0.7555511385430487 0.22879816549162246 0.07697990982879299 0.289751452913768
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  0.5803149133353083 0.8074401551640625 0.8960912999234932 1.0690829462252431 0.11005192452767676 0.22793516254194168 0.6695788847193422 0.8180147659224931 0.8607305832563434
  )");
