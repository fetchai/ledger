...
TEST(blas_gemv, blas_gemv_n_novector3) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_n_novector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.5192124475613537);
  Type beta = Type(0.7727136267182124);
  int n = 1;
  int m = 1;
 ...
 0.6075448519014384 0.17052412368729153 0.06505159298527952 0.9488855372533332
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.940911243766841; 0.7521129823378456; 0.5288872861128661
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_n_novector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_n_novector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.7094521590949134);
  Type beta = Type(0.7141735359225003);
  int n = 2;
  int m = 3;
 ...
 0.3584657285442726 0.11586905952512971 0.8631034258755935 0.6232981268275579
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.8266807675539728; 0.713244787222995; 0.7607850486168974; 1.1734590460342313; 0.770967179954561; 0.49379559636439074; 1.3221237347102763; 0.42754101835854963; 0.02541912674409519
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_n_novector6) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_n_novector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.002437994991828951);
  Type beta = Type(0.8836680888019255);
  int n = -2;
  int m = -3;
 ...
 0.7555511385430487 0.22879816549162246 0.07697990982879299 0.289751452913768
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.47769485817965324; 0.8074401551640625; 0.8960912999234932; 0.2838215382556608; 0.11005192452767676; 0.22793516254194168; 0.37891478475062207; 0.8180147659224931; 0.8607305832563434
  )");
  TensorView< Type > refy = ref_tensor_y.View();
