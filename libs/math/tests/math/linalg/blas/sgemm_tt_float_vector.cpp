...
TEST(blas_SGEMM_vectorised, blas_sgemm_tt_float_vector1) {

	Blas< float, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::VECTORISE> sgemm_tt_float_vector;
 ...
 0.17763558461246698 0.5504679890644331 0.16636834727714633
	...
	sgemm_tt_float_vector(alpha, A, B, beta, C);

	ASSERT_TRUE( R.AllClose(C) ); 
}

TEST(blas_SGEMM_vectorised, blas_sgemm_tt_float_vector2) {
...
TEST(blas_SGEMM_vectorised, blas_sgemm_tt_float_vector3) {

	Blas< float, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::VECTORISE> sgemm_tt_float_vector;
	// Compuing _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C  

  float alpha = float(0.5538901732407187), beta = float(0.34356755290364094);
 ...
 0.388677289689482 0.2713490317738959 0.8287375091519293
  ...
  Matrix< float > R = Matrix< float >(R"(
	0.28366751508177507 0.619640496407821 0.3766681699491577;
 0.37460724512536153 0.6377126985186038 0.7394596522940399;
 0.2651492789237033 0.455488003429968 0.6771555193191199
	)");
