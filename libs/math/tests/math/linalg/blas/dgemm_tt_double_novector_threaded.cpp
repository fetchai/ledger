...
TEST(blas_DGEMM, blas_dgemm_tt_double_novector_threaded1) {

	Blas< double, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::THREADING> dgemm_tt_double_novector_threaded;
 ...
 0.17763558461246698 0.5504679890644331 0.16636834727714633
	...
	dgemm_tt_double_novector_threaded(alpha, A, B, beta, C);

	ASSERT_TRUE( R.AllClose(C) ); 
}

TEST(blas_DGEMM, blas_dgemm_tt_double_novector_threaded2) {
...
TEST(blas_DGEMM, blas_dgemm_tt_double_novector_threaded3) {
...
TEST(blas_DGEMM, blas_dgemm_tt_double_novector_threaded4) {

	Blas< double, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::THREADING> dgemm_tt_double_novector_threaded;
	// Compuing _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C  

  double alpha = 0.043133169224866874, beta = 0.3513801869744796;
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Matrix< double > R = Matrix< double >(R"(
	0.2760388904451542 0.2741005022181137 0.04286270213315593;
 0.1646371231520015 0.04331312051886881 0.3376170567847135;
 0.24459909565165444 0.12094034360500455 0.043694605225244904
	)");
