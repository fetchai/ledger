...
TEST(blas_DGEMM, blas_gemm_nt_novector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.832526270382494);
  Type beta = Type(0.9667774821535076);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  1.178516311172483 0.8059581985278688 0.47910564422294666;
 0.8830429091050706 0.20244895253096368 1.2857922709274514;
 1.3096127508251483 0.45295977149016164 0.6499241083024342
  )");
  TensorView< Type > refC = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.35482417656422416);
  Type beta = Type(0.3077093720490257);
 ...
 0.907566473926093 0.24929222914887494 0.41038292303562973 0.7555511385430487
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.3319993334558072 0.22481635954154158 0.25043033981673946 0.23064784865163884;
 0.26321897380029124 0.18604390788384384 0.021620834982787787 0.11549545839785835;
 0.23935438564817435 0.31807735045421087 0.12769106852317974 0.34113814186528246;
 0.4799726904265418 0.18353439569019075 0.1533333781108726 0.39384097616700064
  )");
  TensorView< Type > refC = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector6) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.860806159526442);
  Type beta = Type(0.6242880375943015);
 ...
 0.9082658859666537 0.23956189066697242 0.1448948720912231 0.489452760277563 0.9856504541106007
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.9138390784964303 0.5538604449779474 0.4805748472006556 0.5138488888438874 0.5521447969713156;
 1.5485103370883966 1.951766061392431 1.0688782917197992 1.1194850269006658 2.0054382712238112;
 1.564946451003588 1.4039442687413413 0.6011151507930192 1.3172140796433205 1.8176899085954843
  )");
  TensorView< Type > refC = refC_tensor.View();
