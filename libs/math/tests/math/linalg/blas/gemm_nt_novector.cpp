...
TEST(blas_DGEMM, blas_gemm_nt_novector4)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.9959946731871625);
  Type beta  = Type(0.3530141619560808);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.8240961251045277 0.3444230275673468 0.51367740511813;
 0.7683700860476008 0.14908878076239848 0.8446772469288317;
 1.0658806113538775 0.2759923515534755 0.7264631688028926
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector5)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.7187816242959871);
  Type beta  = Type(0.1752783378424395);
 ...
 0.907566473926093 0.24929222914887494 0.41038292303562973 0.7555511385430487
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.3316663643588546 0.20393317070357375 0.16186655368571062 0.24598212200503478;
 0.2989971184323681 0.1853122106622105 0.032408878604399616 0.18562186836080044;
 0.4707876283718193 0.3591914583304763 0.11781830693549794 0.4631866160325986;
 0.5656549310473479 0.26009490892028797 0.12673704528904609 0.4592865811592321
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector6)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.7134592546770264);
  Type beta  = Type(0.8798271071866534);
 ...
 0.9082658859666537 0.23956189066697242 0.1448948720912231 0.489452760277563 0.9856504541106007
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  1.1062056671806915 0.5503004162040931 0.5785164033332597 0.5349301804808435 0.560858746171554;
 1.2968147847886902 1.8385824814976726 1.068086113831498 0.946515165747155 1.7631426743893415;
 1.6262255720318288 1.2504440311609162 0.5507303375774087 1.2691202785470752 1.8637503350949691
  )");
  TensorView<Type> refC        = refC_tensor.View();
