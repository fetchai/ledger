...
TEST(blas_DGEMM, blas_gemm_nt_novector4)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.5806862711298963);
  Type beta  = Type(0.16706167338714428);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.45221439703836397 0.1709168544204902 0.29661548784075104;
 0.43408457180068805 0.08243166685968856 0.4590169790992853;
 0.5972765012018847 0.1480861204933352 0.42108053232445875
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector5)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.07428865326813161);
  Type beta  = Type(0.30468489093379025);
 ...
 0.907566473926093 0.24929222914887494 0.41038292303562973 0.7555511385430487
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.25229650456939673 0.18192200713943862 0.2376649721663255 0.16692976507001006;
 0.18070157577088497 0.14167278804774946 0.010633913610919504 0.050103049441408545;
 0.05766422435052343 0.21949937531377484 0.10226170059449716 0.19361271466326765;
 0.3185431029289565 0.09832122616878763 0.13070185261192194 0.26398663303579817
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nt_novector6)
{
  ...
  // Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;
  Type alpha = Type(0.6581640996925654);
  Type beta  = Type(0.16181773299581925);
 ...
 0.9082658859666537 0.23956189066697242 0.1448948720912231 0.489452760277563 0.9856504541106007
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.39505380751081576 0.3440373136332277 0.2105575711885343 0.2979547325307279 0.3322954563275932;
 1.1723380050278238 1.2999797880039297 0.6586553536609786 0.8397055698869775 1.4454239101645043;
 0.909978841881524 0.9978588406682302 0.41389152509320754 0.8527034826111605 1.0788086912362547
  )");
  TensorView<Type> refC        = refC_tensor.View();
