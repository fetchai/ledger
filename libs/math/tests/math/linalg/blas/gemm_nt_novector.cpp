...
TEST(blas_DGEMM, blas_gemm_nt_novector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.8796062662644812);
  Type beta = Type(0.1197106940442364);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > refC = Tensor< Type >::FromString(R"(
  0.5877882644765933 0.15605135858989622 0.43943049916111676;
 0.6097372062587972 0.10941396946324766 0.580210964003311;
 0.8216202510060755 0.18019140685779156 0.629364782539213
  )");
...
TEST(blas_DGEMM, blas_gemm_nt_novector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.2484799688941588);
  Type beta = Type(0.49986836402746804);
 ...
 0.907566473926093 0.24929222914887494 0.41038292303562973 0.7555511385430487
  ...
  Tensor< Type > refC = Tensor< Type >::FromString(R"(
  0.4488498593144946 0.3170541206404516 0.39462349449398576 0.30194727407948674;
 0.33298576170003574 0.25186992338824427 0.022369616755877016 0.11156278955086996;
 0.17655548995192877 0.40373046512806043 0.17881815226613676 0.38352434459284057;
 0.5942162424990731 0.19942171027665984 0.2240835919764861 0.4906685366621403
  )");
...
TEST(blas_DGEMM, blas_gemm_nt_novector6) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nt_novector;
	// Compuing _C <= _alpha * _A * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.8914742307675645);
  Type beta = Type(0.5239768547099313);
 ...
 0.9082658859666537 0.23956189066697242 0.1448948720912231 0.489452760277563 0.9856504541106007
  ...
  Tensor< Type > refC = Tensor< Type >::FromString(R"(
  0.828445963013037 0.5427362926951076 0.43675716962883293 0.49528238544281344 0.5369081466643426;
 1.599158750867347 1.9465981179428247 1.0453546649102519 1.153060224826116 2.042737422990448;
 1.509390519045125 1.424603826134804 0.6047739007590985 1.3041588648997675 1.7616548115136839
  )");
