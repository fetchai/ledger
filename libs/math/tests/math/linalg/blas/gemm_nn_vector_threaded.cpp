...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector_threaded4) {
  ...
  double alpha = double(0.44298414234528305), beta = double(0.6117143371555813);
...
Matrix< double > C = Matrix< double >(R"(
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Matrix< double > R = Matrix< double >(R"(
	0.6025950909714873 0.6953234483815325 0.16466648592648236;
 0.45687665881589445 0.3074378259916901 0.6198722569012041;
 0.7321634432572044 0.5037707975517814 0.13283905546540187
	)");
...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector_threaded7) {
  ...
  double alpha = double(0.6457934219330558), beta = double(0.7031243001523811);
...
Matrix< double > C = Matrix< double >(R"(
 ...
 0.690937738102466 0.3867353463005374 0.9367299887367345 0.13752094414599325
  ...
  Matrix< double > R = Matrix< double >(R"(
	0.38093445372561696 1.2184217803045043 1.0802501966473386 0.572048618840025;
 0.18922298346609423 0.6657352916595582 0.7267335197111509 0.20541281329010486;
 0.6475233555050208 0.6201919758123513 0.9398455507871908 0.47209715296747873;
 0.9630300102703517 0.9045511522908243 1.0734021006099106 0.6502769045263866
	)");
