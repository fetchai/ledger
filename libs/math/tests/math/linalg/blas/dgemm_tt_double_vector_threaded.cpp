...
TEST(blas_DGEMM_vectorised, blas_dgemm_tt_double_vector_threaded1) {

	Blas< double, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::VECTORISE | platform::Parallelisation::THREADING> dgemm_tt_double_vector_threaded;
 ...
 0.17763558461246698 0.5504679890644331 0.16636834727714633
	...
	dgemm_tt_double_vector_threaded(alpha, A, B, beta, C);

	ASSERT_TRUE( R.AllClose(C) ); 
}

TEST(blas_DGEMM_vectorised, blas_dgemm_tt_double_vector_threaded2) {
...
TEST(blas_DGEMM_vectorised, blas_dgemm_tt_double_vector_threaded3) {

	Blas< double, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::VECTORISE | platform::Parallelisation::THREADING> dgemm_tt_double_vector_threaded;
	// Compuing _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C  

  double alpha = 0.19448274387896536, beta = 0.3102118287885177;
 ...
 0.388677289689482 0.2713490317738959 0.8287375091519293
  ...
  Matrix< double > R = Matrix< double >(R"(
	0.21295036975321727 0.3923360558644997 0.1490325103958496;
 0.16868666972510113 0.23248872741341306 0.3213156096465116;
 0.16678422580336333 0.2113734006245368 0.3948741537343886
	)");
