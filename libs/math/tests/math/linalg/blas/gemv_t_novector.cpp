...
TEST(blas_gemv, blas_gemv_t_novector3) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_t_novector;
	// Compuing _y <= _alpha * T(_A) * _x + _beta * _y
  using Type = double;Type alpha = Type(0.31216756908813925);
  Type beta = Type(0.0253237195414151);
  int n = 1;
  int m = 1;
 ...
 0.17052412368729153 0.06505159298527952 0.9488855372533332
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.3650668515747544; 0.17837123396779483; 0.35704095039301875
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_t_novector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_t_novector;
	// Compuing _y <= _alpha * T(_A) * _x + _beta * _y
  using Type = double;Type alpha = Type(0.20298452139806278);
  Type beta = Type(0.5379205962698249);
  int n = 2;
  int m = 3;
 ...
 0.11586905952512971 0.8631034258755935 0.6232981268275579
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.30276847819666747; 0.713244787222995; 0.7607850486168974; 0.5262924306012088; 0.770967179954561; 0.49379559636439074; 0.4929163173436452; 0.42754101835854963; 0.02541912674409519
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_t_novector6) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemv_t_novector;
	// Compuing _y <= _alpha * T(_A) * _x + _beta * _y
  using Type = double;Type alpha = Type(0.2959022906314299);
  Type beta = Type(0.1453466667024338);
  int n = -2;
  int m = -3;
 ...
 0.22879816549162246 0.07697990982879299 0.289751452913768
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.5420509326754614; 0.8074401551640625; 0.8960912999234932; 0.2809045989112145; 0.11005192452767676; 0.22793516254194168; 0.21962400423254205; 0.8180147659224931; 0.8607305832563434
  )");
  TensorView< Type > refy = ref_tensor_y.View();
