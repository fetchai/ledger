...
TEST(blas_gemv, blas_gemv_t_vector3)
{
  ...
  // Compuing _y = _alpha * T(_A) * _x + _beta * _y
  using type = double;
  type alpha = type(0.7408152300964884);
  type beta  = type(0.47289068362919084);
  int  n     = 1;
  int  m     = 1;
 ...
 0.17052412368729153 0.06505159298527952 0.9488855372533332
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  1.1487996473422384 0.6049910645983977 0.8976824083806779
  )");
...
TEST(blas_gemv, blas_gemv_t_vector5)
{
  ...
  // Compuing _y = _alpha * T(_A) * _x + _beta * _y
  using type = double;
  type alpha = type(0.3491599686166341);
  type beta  = type(0.1900374376203967);
  int  n     = 2;
  int  m     = 3;
 ...
 0.11586905952512971 0.8631034258755935 0.6232981268275579
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  0.4328690047810405 0.713244787222995 0.7607850486168974 0.49260924299799763 0.770967179954561 0.49379559636439074 0.46353795140726795 0.42754101835854963 0.02541912674409519
  )");
...
TEST(blas_gemv, blas_gemv_t_vector6)
{
  ...
  // Compuing _y = _alpha * T(_A) * _x + _beta * _y
  using type = double;
  type alpha = type(0.6188015793076526);
  type beta  = type(0.9098335587277321);
  int  n     = -2;
  int  m     = -3;
 ...
 0.22879816549162246 0.07697990982879299 0.289751452913768
  ...
  ShapeLessArray<type> refy = ShapeLessArray<type>(R"(
  1.4603328544570022 0.8074401551640625 0.8960912999234932 0.7801096019926539 0.11005192452767676 0.22793516254194168 0.718061455096838 0.8180147659224931 0.8607305832563434
  )");
