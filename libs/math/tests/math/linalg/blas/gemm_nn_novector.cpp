...
TEST(blas_DGEMM, blas_gemm_nn_novector4)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.8653317247298928);
  Type beta  = Type(0.2504731579891035);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.488601000836411 0.6298221122812664 0.2517298216717479;
 0.5539135281825794 0.49112003075646526 0.3957021056388702;
 0.841539535342664 0.6715537819760051 0.19946152956394278
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nn_novector7)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(-1.1617720597813408);
  Type beta  = Type(-4.375230608046806);
 ...
 0.1014715428660321 0.6635017691080558 0.005061583846218687 0.16080805141749865 0.5487337893665861 0.6918951976926933 0.6519612595026005 0.22426930946055978 0.7121792213475359
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  -1.7371152510798926 -1.8714842901376694 -1.3875300978112703 -4.356450097701417 -5.05233028312162 -1.510627288089258 -3.7848364978436893 -4.041572749269872 -3.837783135659669;
 -3.0242446049499705 -1.2492692786322592 -0.9578901487001832 -4.127779878754829 -4.678229586978018 -2.996329631631705 -2.0753163347986474 -1.8413282936136373 -4.073853067128617;
 -4.671608833256596 -4.4463523276961965 -4.027582998412055 -2.951458564263278 -1.2853508410548313 -1.0391374038661156 -4.2871732077979505 -2.7961161355077273 -0.8216911054524298;
 -1.2684154829547485 -3.4460405964800644 -0.694229656585777 -0.8767365397068818 -3.3831902017908755 -3.3730911655228217 -3.3055035030648208 -1.1809709334423841 -4.016102553642609
  )");
  TensorView<Type> refC        = refC_tensor.View();
