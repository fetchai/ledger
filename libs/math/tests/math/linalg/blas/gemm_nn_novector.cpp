...
TEST(blas_DGEMM, blas_gemm_nn_novector1) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
...
TEST(blas_DGEMM, blas_gemm_nn_novector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(0.8859632175771965);
  Type beta = Type(0.025920385250676303);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.3321962635635572 0.4670417066744756 0.24066252219003795;
 0.48448491856975656 0.47611878685461073 0.20616995579911437;
 0.7179181994757539 0.6112850204245305 0.18956538486019756
  )");
  TensorView< Type > refC = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nn_novector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(3.2);
  Type beta = Type(0);
...
TEST(blas_DGEMM, blas_gemm_nn_novector7) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(4.29392486634455);
  Type beta = Type(-0.91244429356159);
 ...
 0.1014715428660321 0.6635017691080558 0.005061583846218687 0.16080805141749865 0.5487337893665861 0.6918951976926933 0.6519612595026005 0.22426930946055978 0.7121792213475359
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  4.071081873273716 1.0904732992686443 3.189821179962413 0.30462829733165564 3.6855803350728524 1.1767901623502484 2.71406431477562 0.9768097809663506 4.699804912455991;
 2.129439454286432 0.4856673522473229 1.9498670571753531 -0.07112582478145757 1.9086375496089167 0.25897732041127297 1.8786305027630685 0.8399027643070374 2.6552565875044714;
 1.9406754283955383 1.2793651480055581 1.5631096077070055 -0.05942431801686454 3.313280834570081 1.0795049608482472 0.4951291392934123 -0.025360383500166117 2.8798646787681603;
 2.9546060410153636 1.401775681978592 2.4794131428779855 0.49328889483212607 3.1301018615319336 0.6470986201646097 1.0794993450270376 0.5336120697607402 2.677158033227032
  )");
  TensorView< Type > refC = refC_tensor.View();
