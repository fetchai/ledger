...
TEST(blas_DGEMM, blas_gemm_nn_novector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(0.505479683934374);
  Type beta = Type(0.2909523733746835);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.39085740296341387 0.4794640168834564 0.1577566454673572;
 0.3754144636691448 0.30364483815355087 0.3559865537355216;
 0.5817351680070637 0.4401461140911129 0.12570762041626637
  )");
  TensorView< Type > refC = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nn_novector7) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::NOT_PARALLEL> gemm_nn_novector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(0.610668400360737);
  Type beta = Type(4.817911105275721);
 ...
 0.1014715428660321 0.6635017691080558 0.005061583846218687 0.16080805141749865 0.5487337893665861 0.6918951976926933 0.6519612595026005 0.22426930946055978 0.7121792213475359
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  1.2593854562839084 1.8425694747999122 1.0150764922341797 4.618407735220661 4.86494246826185 1.4435709834877304 3.6513728883803864 4.182269497128769 3.4153451307036065;
 2.9233816158219175 1.2656768591376828 0.7379462536933991 4.429017622301642 4.726417240764372 3.1692118481853417 1.9445824579232145 1.847224158739216 3.969415706953721;
 4.714607657438675 4.570963511303076 4.08087205530268 3.168113456790414 0.8875015999303488 0.9532102946770116 4.516170731472876 2.996807857410164 0.45506926921384067;
 0.922243038956012 3.4821479077788036 0.3776573647501066 0.8657801286010627 3.1601102013442843 3.515301611693917 3.3792160845213 1.1855005090220778 3.9043690123703643
  )");
  TensorView< Type > refC = refC_tensor.View();
