...
TEST(blas_DGEMM, blas_gemm_nn_novector4)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.08769943041352457);
  Type beta  = Type(0.8511031548015668);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.6514731476917414 0.7006830691558728 0.08665227422373388;
 0.3521295925953868 0.14544911255283233 0.7527837818487252;
 0.5999567911899382 0.3412890122205843 0.072696269389045
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_DGEMM, blas_gemm_nn_novector7)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(-2.602952521014884);
  Type beta  = Type(2.627920011690068);
 ...
 0.1014715428660321 0.6635017691080558 0.005061583846218687 0.16080805141749865 0.5487337893665861 0.6918951976926933 0.6519612595026005 0.22426930946055978 0.7121792213475359
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  -2.1825377159595227 0.04660635634095278 -1.6982161897222348 1.7338918209045862 -0.4138725316992704 -0.17818573258022397 -0.275914241789319 1.1034374139159229 -1.6970631700730492;
 -0.19193409780341697 0.2073867532978051 -0.9888284131397977 1.9046606985643098 0.71118367974717 1.1565693164494455 -0.4353954584605759 0.21539634411114061 -0.10338729106900058;
 0.6848994794289318 1.0649844925739111 0.6705389402076667 1.3681109252869046 -1.8339180287913304 -0.3190421849660669 1.564177178775966 1.2735932632245621 -1.7266753234518526;
 -1.580531850503125 0.5268861458821987 -1.4925041838765873 0.03461590547086796 -0.7589360097423727 1.0432777630785277 0.6983039379712581 0.1418417447927064 -0.14524723655096272
  )");
  TensorView<Type> refC        = refC_tensor.View();
