...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector4)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using type = double;
  type alpha = type(0.08730951350231764);
  type beta  = type(0.031089181543611955);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<type> refC = Tensor<type>::FromString(R"(
  0.05353919832044716 0.06803385870249681 0.02582955303726687;
 0.057973551249034055 0.050226644176698754 0.04494602183845967;
 0.08853476863425751 0.06968275325934217 0.020494830402901714
  )");
...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector7)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using type = double;
  type alpha = type(0.02219736926758631);
  type beta  = type(0.8462291246677243);
 ...
 0.690937738102466 0.3867353463005374 0.9367299887367345 0.13752094414599325
  ...
  Tensor<type> refC = Tensor<type>::FromString(R"(
  0.0873172856153312 0.7285496332863646 0.30083145740925366 0.1729922187300318;
 0.04002367896236918 0.5086329570270519 0.5819788103667496 0.020696709365353422;
 0.44322864677814744 0.20751077080877894 0.5626760962323107 0.15956690834474777;
 0.6010945843291042 0.3490115623035719 0.8069445845511051 0.13540210410724124
  )");
