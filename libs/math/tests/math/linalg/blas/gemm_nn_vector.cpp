...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector4)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.4112693523354246);
  Type beta  = Type(0.7889334743097759);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.7205739204650342 0.8160042797498276 0.16924233238185873;
 0.5033931685492785 0.31103594703530596 0.7662512922515936;
 0.8175028505481599 0.5408371451554824 0.1373759208809107
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector7)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.18839298451912867);
  Type beta  = Type(0.4344955206635349);
 ...
 0.690937738102466 0.3867353463005374 0.9367299887367345 0.13752094414599325
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.1318379211003558 0.5470418506396089 0.38871396638207456 0.20966311011314792;
 0.06455366369541565 0.3297482060350154 0.3674233079741504 0.06372858202797738;
 0.3063604170889638 0.2328775451994832 0.4221631448875856 0.17771752171814645;
 0.4394240540488479 0.352587138587491 0.5280014454950966 0.22124517018199807
  )");
  TensorView<Type> refC        = refC_tensor.View();
