...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_nn_vector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(0.014381163392392371);
  Type beta = Type(0.026755523003665127);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.02459052926373415 0.027892363615904307 0.005856440089125111;
 0.01730438828454325 0.010779858035244245 0.026076235660326602;
 0.02806783442841581 0.01863664803573692 0.004750864921715109
  )");
  TensorView< Type > refC = refC_tensor.View();
...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector7) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_nn_vector;
	// Compuing _C <= _alpha * _A * _B + _beta * _C
  using Type = double;Type alpha = Type(0.6418009189559176);
  Type beta = Type(0.732994440151096);
 ...
 0.690937738102466 0.3867353463005374 0.9367299887367345 0.13752094414599325
  ...
  Tensor< Type > refC_tensor = Tensor< Type >::FromString(R"(
  0.3816688465838971 1.239470717439438 1.0845478993736097 0.5748941489586166;
 0.18944835234268848 0.6818381332799947 0.7454248935819214 0.20471047218942814;
 0.6610424871429013 0.6241077734938513 0.9561110524063748 0.47514480649491386;
 0.9807181210112834 0.912191880310127 1.0988181488648743 0.6509622484914885
  )");
  TensorView< Type > refC = refC_tensor.View();
