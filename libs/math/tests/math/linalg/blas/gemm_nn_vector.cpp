...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector4)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using type = double;
  type alpha = type(0.9998626749756515);
  type beta  = type(0.10171197514922714);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<type> refC = Tensor<type>::FromString(R"(
  0.42772674815390666 0.5829702842468142 0.27696732414310415;
 0.5727444770042954 0.5457244189645337 0.2952150107462126;
 0.8553775081631183 0.7138484681648192 0.21854130764088625
  )");
...
TEST(blas_gemm_vectorised, blas_gemm_nn_vector7)
{
  ...
  // Compuing _C <= _alpha * _A * _B + _beta * _C
  using type = double;
  type alpha = type(0.01950049285414812);
  type beta  = type(0.7184893844544453);
 ...
 0.690937738102466 0.3867353463005374 0.9367299887367345 0.13752094414599325
  ...
  Tensor<type> refC = Tensor<type>::FromString(R"(
  0.0744580034610316 0.6192127925539802 0.25628580293067604 0.14732515265097385;
 0.03414459297314369 0.43210732637375704 0.4943815553062154 0.017768673537518043;
 0.37661356758170705 0.17665335444161467 0.4782314508559982 0.13583386789225063;
 0.5108414959147471 0.29696819234906163 0.6855548549889281 0.11552341661348552
  )");
