...
TEST(blas_A_withA_vectorised, blas_syrk_ut_vector_threaded1) {

	Blas< double, 
        Signature( U(_C) <= _alpha, U(_A), _beta, U(_C) ), 
        Computes( _C = _alpha * T(_A) * _A + _beta * _C ), 
        platform::Parallelisation::VECTORISE | platform::Parallelisation::THREADING> syrk_ut_vector_threaded;
...
TEST(blas_A_withA_vectorised, blas_syrk_ut_vector_threaded2) {
...
TEST(blas_A_withA_vectorised, blas_syrk_ut_vector_threaded3) {
  ...
  double alpha = double(0.7108317801140336), beta = double(0.9944275087769094);
...
Matrix< double > C = Matrix< double >(R"(
 ...
 0.034388521115218396 0.9093204020787821 0.2587799816000169
  ...
  Matrix< double > R = Matrix< double >(R"(
	1.205313353921559 0.8220876290361914 1.2537758582232184;
 0.8220876290361914 0.8048397811870142 1.0551882083489947;
 1.2537758582232184 1.0551882083489947 0.7248789931537419
	)");
