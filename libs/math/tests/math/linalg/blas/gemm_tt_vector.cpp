...
TEST(blas_gemm_vectorised, blas_gemm_tt_vector3) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_tt_vector;
	// Compuing _C <= _alpha * T(_A) * T(_B) + _beta * _C
  using Type = double;Type alpha = Type(0.19569374240734683);
  Type beta = Type(0.6998795525193885);
 ...
 0.388677289689482 0.2713490317738959 0.8287375091519293
  ...
  Tensor< Type > refC = Tensor< Type >::FromString(R"(
  0.44610377219062697 0.7522229679464486 0.18427223922651803;
 0.24572667430449757 0.2514726332509914 0.44945868277455914;
 0.31852697143341024 0.31790139061257916 0.7186643984239621
  )");
