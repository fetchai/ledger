...
TEST(blas_gemv, blas_gemv_n_vector3) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemv_n_vector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.4542128390810528);
 ...
 0.6075448519014384 0.17052412368729153 0.06505159298527952 0.9488855372533332
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.6498372871707184; 0.5464879815562244; 0.43177025079714626
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_n_vector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemv_n_vector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.6629966159642487);
 ...
 0.3584657285442726 0.11586905952512971 0.8631034258755935 0.6232981268275579
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.7297440162784624; 0.713244787222995; 0.7607850486168974; 0.8957285116649709; 0.770967179954561; 0.49379559636439074; 1.0484542447854377; 0.42754101835854963; 0.02541912674409519
  )");
  TensorView< Type > refy = ref_tensor_y.View();
...
TEST(blas_gemv, blas_gemv_n_vector6) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemv_n_vector;
	// Compuing _y <= _alpha * _A * _x + _beta * _y
  using Type = double;Type alpha = Type(0.07726693689233366);
 ...
 0.7555511385430487 0.22879816549162246 0.07697990982879299 0.289751452913768
  ...
  Tensor< Type > ref_tensor_y = Tensor< Type >::FromString(R"(
  0.21101313333555638; 0.8074401551640625; 0.8960912999234932; 0.19306898869672282; 0.11005192452767676; 0.22793516254194168; 0.18693774458047893; 0.8180147659224931; 0.8607305832563434
  )");
  TensorView< Type > refy = ref_tensor_y.View();
