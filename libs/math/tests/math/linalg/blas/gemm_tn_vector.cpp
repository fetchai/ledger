...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector4) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_tn_vector;
	// Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;Type alpha = Type(0.27568636699969307);
  Type beta = Type(0.0172033522599756);
...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector5) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_tn_vector;
	// Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;Type alpha = Type(2.604572696395226);
  Type beta = Type(2.7718641791184773);
...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector7) {
	...
	Blas< double, 
        ...
        platform::Parallelisation::VECTORISE> gemm_tn_vector;
	// Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;Type alpha = Type(1.5238231607380632);
  Type beta = Type(1.6746586496235456);
