...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector4)
{
  ...
  // Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.3620613398505078);
  Type beta  = Type(0.9864348585369677);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  0.8468726566494503 0.9021636395976174 0.13477378533591838;
 0.45558868170913003 0.4296928594784354 1.0769106131489088;
 0.8089047467614329 0.5001580162607131 0.12082109861044052
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector5)
{
  ...
  // Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;
  Type alpha = Type(0.40656266475784264);
  Type beta  = Type(2.3266270886273004);
 ...
 0.3143559810763267 0.5085706911647028 0.907566473926093
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  1.3768121308430126 1.2847469041307746 0.2833250815959798;
 0.4692912986017713 0.4455139354999044 1.759386927011932;
 0.8746212371880735 1.5428387005174855 2.4320422198042313
  )");
  TensorView<Type> refC        = refC_tensor.View();
...
TEST(blas_gemm_vectorised, blas_gemm_tn_vector7)
{
  ...
  // Compuing _C <=  _alpha * T(_A) * _B + _beta * _C
  using Type = double;
  Type alpha = Type(1.429180897303852);
  Type beta  = Type(2.3579042692871592);
 ...
 0.9367299887367345 0.13752094414599325 0.3410663510502585 0.11347352124058907 0.9246936182785628 0.877339353380981 0.2579416277151556
  ...
  Tensor<Type>     refC_tensor = Tensor<Type>::FromString(R"(
  2.843625972780197 1.477174752822079 0.778473467358419 2.483679886812668 3.6088244868119568 0.8814406088537927 2.254611439415921;
 2.4855543367521062 1.4739339396333175 2.1706987268797513 2.1670491808624086 2.7609748393548474 1.812348597910724 1.9227252753098631;
 0.36322700332162355 2.113987698148745 0.7777800919610244 0.7435710841863901 0.3482558551299591 1.4206098397102438 1.737023073342563;
 0.8232640377557862 2.1044830851021827 0.849195315776462 3.052050197138766 1.7910825170350146 1.8823257769519675 1.6503523953073354;
 2.6571875524112385 0.8752235082316886 1.0381658163701268 1.1288491566219538 2.9866877875714875 2.242663234804202 1.0335492700732716
  )");
  TensorView<Type> refC        = refC_tensor.View();
