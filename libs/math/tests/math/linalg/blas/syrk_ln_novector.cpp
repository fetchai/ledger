...
TEST(blas_A_withA, blas_syrk_ln_novector1) {

	Blas< double, 
        Signature( L(_C) <= _alpha, L(_A), _beta, L(_C) ), 
        Computes( _C = _alpha * _A * T(_A) + _beta * _C ), 
        platform::Parallelisation::NOT_PARALLEL> syrk_ln_novector;
...
Matrix< double > C = Matrix< double >(R"(
 ...
 0.20674146233889457 0.20759214270103027 0.04867610654042823
	...
	syrk_ln_novector(alpha, A, beta, C);
...
TEST(blas_A_withA, blas_syrk_ln_novector2) {
...
TEST(blas_A_withA, blas_syrk_ln_novector3) {
  ...
  double alpha = double(0.4649096599278447), beta = double(0.8674434604889388);
...
Matrix< double > C = Matrix< double >(R"(
 ...
 0.034388521115218396 0.9093204020787821 0.2587799816000169
  ...
  Matrix< double > R = Matrix< double >(R"(
	0.44935723868265076 0.4754074945213763 0.36666459418324604;
 0.4754074945213763 0.5264257404583412 1.1746090548246484;
 0.36666459418324604 1.1746090548246484 0.9618011732915892
	)");
