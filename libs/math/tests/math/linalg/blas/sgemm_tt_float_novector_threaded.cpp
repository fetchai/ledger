...
TEST(blas_SGEMM, blas_sgemm_tt_float_novector_threaded1) {

	Blas< float, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::THREADING> sgemm_tt_float_novector_threaded;
 ...
 0.17763558461246698 0.5504679890644331 0.16636834727714633
	...
	sgemm_tt_float_novector_threaded(alpha, A, B, beta, C);

	ASSERT_TRUE( R.AllClose(C) ); 
}

TEST(blas_SGEMM, blas_sgemm_tt_float_novector_threaded2) {
...
TEST(blas_SGEMM, blas_sgemm_tt_float_novector_threaded3) {
...
TEST(blas_SGEMM, blas_sgemm_tt_float_novector_threaded4) {

	Blas< float, Computes( _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C ),platform::Parallelisation::THREADING> sgemm_tt_float_novector_threaded;
	// Compuing _C <= _C = _alpha * T(_A) * T(_B) + _beta * _C  

  float alpha = float(0.5887606245378306), beta = float(0.6723735669246739);
 ...
 0.6232981268275579 0.3308980248526492 0.06355835028602363
  ...
  Matrix< float > R = Matrix< float >(R"(
	0.7615268018320631 0.5607786686939785 0.2797154723803706;
 0.7689898610170645 0.11338360490592113 1.0490579743324144;
 0.768313149562509 0.2862227186877704 0.334315352906981
	)");
