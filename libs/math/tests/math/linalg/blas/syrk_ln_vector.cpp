...
TEST(blas_A_withA_vectorised, blas_syrk_ln_vector1) {

	Blas< double, 
        Signature( L(_C) <= _alpha, L(_A), _beta, L(_C) ), 
        Computes( _C = _alpha * _A * T(_A) + _beta * _C ), 
        platform::Parallelisation::VECTORISE> syrk_ln_vector;
...
TEST(blas_A_withA_vectorised, blas_syrk_ln_vector2) {
...
TEST(blas_A_withA_vectorised, blas_syrk_ln_vector3) {
  ...
  type alpha = type(0.3525183055586444);
  type beta = type(0.10885787328652052);
...
Matrix< type > C = Matrix< type >(R"(
 ...
 0.034388521115218396 0.9093204020787821 0.2587799816000169
  ...
  Matrix< type > R = Matrix< type >(R"(
	0.17352859756271155 0.11888644623061154 0.2591486209608133;
 0.11888644623061154 0.3321783362732875 0.3915389667760219;
 0.2591486209608133 0.3915389667760219 0.5872471724230707
	)");
