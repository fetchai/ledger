...
void Blas< S, Signature( _y <= _alpha, _A, _x, _n, _beta, _y, _m ), Computes( _y = _alpha * T(_A) * _x + _beta * _y ),V >::operator()(type const &alpha, Matrix< type > const &a, ShapeLessArray< type > const &x, int const &incx, type const &beta, ShapeLessArray< type > &y, int const &incy ) const
{
  int jy;
  type temp;
  int kx;
  ...
  if( incx == 1 ) 
  {
    for(j = 0 ; j <  int(a.width()); ++j)
    {
      ...
      temp = slice_a_j.in_parallel().SumReduce(range, [](vector_register_type const &vr_a_j, vector_register_type const &vr_v_x) {
