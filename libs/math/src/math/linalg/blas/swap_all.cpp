...
void Blas< S, Signature( _x, _y <= _n, _x, _m, _y, _p ), Computes( _x, _y <= _y, _x ),V >::operator()(int const n, TensorView< Type >dx, int const incx, TensorView< Type >dy, int const incy ) const
{
  Type dtemp;
  int i;
  if( (incx == 1) && (incy == 1) ) 
