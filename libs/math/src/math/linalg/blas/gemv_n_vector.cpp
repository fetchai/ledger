...
void Blas< S, Signature( _y <= _alpha, _A, _x, _n, _beta, _y, _m ), Computes( _y = _alpha * _A * _x + _beta * _y ),V >::operator()(type const &alpha, Matrix< type > const &a, ShapeLessArray< type > const &x, int const &incx, type const &beta, ShapeLessArray< type > &y, int const &incy ) const
{
  int iy;
  type temp;
  int kx;
  ...
  if( beta != 1.0 ) 
  {
    if( incy == 1 ) 
    {
      if( beta == 0.0 ) 
      {
        ...
        ret_slice.in_parallel().Apply(range, [vec_zero](vector_register_type &vw_v_y ){
      ...
      else 
      {
        ...
        memory::TrivialRange range( std::size_t(0), std::size_t(leny) );
        ret_slice.in_parallel().Apply(range, [vec_beta](vector_register_type const &vr_v_y, vector_register_type &vw_v_y ){
  ...
  if( incy == 1 ) 
  {
    for(j = 0 ; j <  int(a.width()); ++j)
    {
      ...
      ret_slice.in_parallel().Apply(range, [vec_temp](vector_register_type const &vr_v_y, vector_register_type const &vr_a_j, vector_register_type &vw_v_y ){
