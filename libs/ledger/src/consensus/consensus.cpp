...
Consensus::Consensus(StakeManagerPtr stake, BeaconSetupServicePtr beacon_setup,
  ...
  , consensus_total_validate_block_failures_total_{telemetry::Registry::Instance().CreateCounter(
        "consensus_total_validate_block_failures_total", "The total number of DKG failures")}
  , consensus_total_non_heaviest_blocks_total_{telemetry::Registry::Instance().CreateCounter(
        "consensus_total_non_heaviest_blocks_total", "The total number of DKG failures")}
{
  assert(stake_);
}
...
NextBlockPtr Consensus::GenerateNextBlock()
{
  ...
  if(ret->weight != GetBeginningOfAeon(*current, chain_).block_entropy.qualified.size())
  {
    consensus_total_non_heaviest_blocks_total_->add(1);
  }
...
Status Consensus::ValidBlock(Block const &current) const
{
  ...
  if (!block_preceeding)
  {
    ...
    consensus_total_validate_block_failures_->add(1);
    return Status::NO;
  }
