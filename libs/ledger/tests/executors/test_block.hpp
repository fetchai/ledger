...
struct TestBlock {
  ...
  void GenerateBlock(uint32_t seed,
    ...
    // generate a series of transactions to be populated in the block. Generation put into loop
    // to catch the rare case when on a single loop no transactions are generated.
...
//            fetch::logger.Info("Generating TX: ", fetch::byte_array::ToBase64(summary.transaction_hash));
...
//              fetch::logger.Info(" - Resource: ", index);
  ...
  static TestBlock Generate(std::size_t log2_num_lanes,
                            std::size_t num_slices,
                            uint32_t seed,
