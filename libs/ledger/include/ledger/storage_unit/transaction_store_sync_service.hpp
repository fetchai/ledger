...
class TransactionStoreSyncService : public network::AtomicStateMachine<transaction_store_sync_helper::State>, public VerifiedTransactionSink
{
public:
  ...
  TransactionStoreSyncService(int lane_id, MuddlePtr muddle, ObjectStorePtr store, LaneControllerPtr controller_ptr, std::size_t verification_threads,
  ...
  virtual bool PossibleNewState(State &current_state) override
  {
    ...
    switch(current_state)
    {
      ...
      case State::QUERY_OBJECTS:
      {
        ...
        for (auto const &connection : active_connections)
        {
          auto promise = PromiseOfTxList(client_->CallSpecificAddress(connection, RPC_TX_STORE_SYNC, TransactionStoreSyncProtocol::PULL_OBJECTS));
  ...
  void OnTransactions(TransactionList const &txs) override
  {
    ...
    for(std::size_t b=0;b<num_batches;++b)
    {
      store_->WithLock([this, &txs, &b, &batch_size]() {
        for (std::size_t i=0, end=std::min(batch_size,txs.size()-b*batch_size);i<end;++i)
        {
