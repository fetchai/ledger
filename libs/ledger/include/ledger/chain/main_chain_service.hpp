...
public:
  ...
  using mainchain_protocol_type =
      fetch::chain::MainChainProtocol<client_register_type>;
  ...
  using block_sync_protocol_type =
      storage::ObjectStoreSyncronisationProtocol<client_register_type,
                                                 block_type>;
  ...
  MainChainService(std::string const &db_dir, uint16_t port,
                   fetch::network::NetworkManager tm, bool start_sync = true)
      : super_type(port, tm)
  {

    fetch::logger.Warn("Establishing mainchain Service on rpc://127.0.0.1:",
                       port);
    ...
    mainchain_protocol_.reset(new mainchain_protocol_type(
        CHAIN, register_, thread_pool_, mainchain_.get()));
