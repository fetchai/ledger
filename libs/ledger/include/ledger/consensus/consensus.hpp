...
public:
  ...
  static constexpr char const *LOGGING_NAME = "Consensus";
  ...
  void UpdateCurrentBlock(Block const &current) override
  {
    ...
    if (beacon_)
    {
      if ((current_block_number_ % aeon_period_) == 0)
      {
        ...
        auto const current_stakers = stake_->GetCommittee(current);
        ...
        // Disallow multiple invocations of cabinet generation for now.
        static bool did_genesis_already = false;

        if (current_block_number_ != 0 || !did_genesis_already)
        {
