...
template <>
class VectorRegister<float, 128> : public BaseVectorRegisterType
{
public:
  using type           = float;
...
template <>
class VectorRegister<float, 256> : public BaseVectorRegisterType
{
public:
  using type           = float;
...
{
  return {_mm_sub_ps(_mm_setzero_ps(), x.data())};
}

inline VectorRegister<float, 256> operator-(VectorRegister<float, 256> const &x)
{
  return {_mm256_sub_ps(_mm256_setzero_ps(), x.data())};
}
...
inline VectorRegister<float, 128> vector_zero_below_element(VectorRegister<float, 128> const &a,
                                                            int const &                       n)
{
  alignas(16) const uint32_t mask[4] = {uint32_t(-(int32_t(0 >= n))), uint32_t(-(int32_t(1 >= n))),
                                        uint32_t(-(int32_t(2 >= n))), uint32_t(-(int32_t(3 >= n)))};

  __m128i conv = _mm_castps_si128(a.data());
  conv         = _mm_and_si128(conv, *reinterpret_cast<__m128i const *>(mask));

  return {_mm_castsi128_ps(conv)};
}

inline VectorRegister<float, 256> vector_zero_below_element(VectorRegister<float, 256> const &a,
                                                            int const &                       n)
{
  alignas(32) const uint32_t mask[8] = {uint32_t(-(int32_t(0 >= n))), uint32_t(-(int32_t(1 >= n))),
                                        uint32_t(-(int32_t(2 >= n))), uint32_t(-(int32_t(3 >= n))),
                                        uint32_t(-(int32_t(4 <= n))), uint32_t(-(int32_t(5 <= n))),
                                        uint32_t(-(int32_t(6 <= n))), uint32_t(-(int32_t(7 <= n)))};
...
template<>
inline VectorRegister<float, 256> rotate_elements_left<0>(VectorRegister<float, 256> const &x)
{
  return x;
}

template<>
inline VectorRegister<float, 256> rotate_elements_left<1>(VectorRegister<float, 256> const &x)
{
  __m128 hi = _mm256_extractf128_ps(x.data(), 1);
  __m128 lo = _mm256_extractf128_ps(x.data(), 0);
  __m128i hi1 = _mm_alignr_epi8(_mm_castps_si128(lo), _mm_castps_si128(hi), 4);
  __m128i lo1 = _mm_alignr_epi8(_mm_castps_si128(hi), _mm_castps_si128(lo), 4);

  return {_mm256_castsi256_ps(_mm256_set_m128i(hi1, lo1))};
}

template<>
inline VectorRegister<float, 256> rotate_elements_left<2>(VectorRegister<float, 256> const &x)
{
  __m128 hi = _mm256_extractf128_ps(x.data(), 1);
  __m128 lo = _mm256_extractf128_ps(x.data(), 0);
  __m128i hi1 = _mm_alignr_epi8(_mm_castps_si128(lo), _mm_castps_si128(hi), 8);
  __m128i lo1 = _mm_alignr_epi8(_mm_castps_si128(hi), _mm_castps_si128(lo), 8);

  return {_mm256_castsi256_ps(_mm256_set_m128i(hi1, lo1))};
}

template<>
inline VectorRegister<float, 256> rotate_elements_left<3>(VectorRegister<float, 256> const &x)
{
  __m128 hi = _mm256_extractf128_ps(x.data(), 1);
  __m128 lo = _mm256_extractf128_ps(x.data(), 0);
  __m128i hi1 = _mm_alignr_epi8(_mm_castps_si128(lo), _mm_castps_si128(hi), 12);
  __m128i lo1 = _mm_alignr_epi8(_mm_castps_si128(hi), _mm_castps_si128(lo), 12);

  return {_mm256_castsi256_ps(_mm256_set_m128i(hi1, lo1))};
}

template<>
inline VectorRegister<float, 256> rotate_elements_left<4>(VectorRegister<float, 256> const &x)
{
  __m128 hi = _mm256_extractf128_ps(x.data(), 1);
  __m128 lo = _mm256_extractf128_ps(x.data(), 0);

  return {_mm256_set_m128(lo, hi)};
}
...
inline bool all_less_than(VectorRegister<float, 128> const &x, VectorRegister<float, 128> const &y)
{
  __m128i r = _mm_castps_si128((x < y).data());
  return _mm_movemask_epi8(r) == 0xFFFF;
}

inline bool all_less_than(VectorRegister<float, 256> const &x, VectorRegister<float, 256> const &y)
{
  __m256i r = _mm256_castps_si256((x < y).data());
  return _mm256_movemask_epi8(r) == 0xFFFF;
}

inline bool any_less_than(VectorRegister<float, 128> const &x, VectorRegister<float, 128> const &y)
{
  __m128i r = _mm_castps_si128((x < y).data());
  return _mm_movemask_epi8(r) != 0;
}

inline bool any_less_than(VectorRegister<float, 256> const &x, VectorRegister<float, 256> const &y)
{
  __m256i r = _mm256_castps_si256((x < y).data());
  return _mm256_movemask_epi8(r) != 0;
}

inline bool all_equal_to(VectorRegister<float, 128> const &x, VectorRegister<float, 128> const &y)
{
  __m128i r = _mm_castps_si128((x == y).data());
  return _mm_movemask_epi8(r) == 0xFFFF;
}

inline bool all_equal_to(VectorRegister<float, 256> const &x, VectorRegister<float, 256> const &y)
{
  __m256i r    = _mm256_castps_si256((x == y).data());
...
inline bool any_equal_to(VectorRegister<float, 128> const &x, VectorRegister<float, 128> const &y)
{
  __m128i r = _mm_castps_si128((x == y).data());
  return _mm_movemask_epi8(r) != 0;
}

inline bool any_equal_to(VectorRegister<float, 256> const &x, VectorRegister<float, 256> const &y)
{
  __m256i r = _mm256_castps_si256((x == y).data());
  return _mm256_movemask_epi8(r) != 0;
}
