...
public:
  ...
  {
    return VectorRegister((VectorRegister<int64_t, E_VECTOR_SIZE>(x.data()) == VectorRegister<int64_t, E_VECTOR_SIZE>(fixed_point::fp64_t::NaN.Data())).data());
  }
...
inline VectorRegister<fixed_point::fp64_t, 128> operator~(VectorRegister<fixed_point::fp64_t, 128> const &x)
{
...
inline VectorRegister<fixed_point::fp64_t, 256> operator~(VectorRegister<fixed_point::fp64_t, 256> const &x)
{
    ...
    VectorRegister<fixed_point::fp64_t, 128> const &b)
{
  ...
  VectorRegister<int64_t, 128> ret = (VectorRegister<int64_t, 128>(a.data()) == VectorRegister<int64_t, 128>(b.data()));
    ...
    VectorRegister<fixed_point::fp64_t, 256> const &b)
{
  ...
  VectorRegister<int64_t, 256> ret = (VectorRegister<int64_t, 256>(a.data()) == VectorRegister<int64_t, 256>(b.data()));
  ret.data() = _mm256_blendv_epi8(ret.data(), _mm256_setzero_si256(), (mask_nan_a | mask_nan_b).data());
  return VectorRegister<fixed_point::fp64_t, 256>(ret.data());
}
...
inline VectorRegister<fixed_point::fp64_t, 128> operator-(VectorRegister<fixed_point::fp64_t, 128> const &x)
{
  VectorRegister<fixed_point::fp64_t, 128> mask = VectorRegister<fixed_point::fp64_t, 128>::MaskNaN(x);
  VectorRegister<int64_t, 128> ret = operator-(VectorRegister<int64_t, 128>(x.data()));
  ret.data() = _mm_blendv_epi8(ret.data(), VectorRegister<fixed_point::fp64_t, 128>(fixed_point::fp64_t::NaN).data(), mask.data());
  return VectorRegister<fixed_point::fp64_t, 128>(ret.data());
}

inline VectorRegister<fixed_point::fp64_t, 256> operator-(VectorRegister<fixed_point::fp64_t, 256> const &x)
{
  VectorRegister<fixed_point::fp64_t, 256> mask = VectorRegister<fixed_point::fp64_t, 256>::MaskNaN(x);
  VectorRegister<int64_t, 256> ret = operator-(VectorRegister<int64_t, 256>(x.data()));
  ret.data() = _mm256_blendv_epi8(ret.data(), VectorRegister<fixed_point::fp64_t, 256>(fixed_point::fp64_t::NaN).data(), mask.data());
  return VectorRegister<fixed_point::fp64_t, 256>(ret.data());
}
    ...
    VectorRegister<fixed_point::fp64_t, 128> const &b)
{
  ...
  sum.data() = _mm_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 128>::MaskPosInf().data(), mask_pos_inf.data());
  // Replace -inf in the final product using the mask
  sum.data() = _mm_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 128>::MaskNegInf().data(), mask_neg_inf.data());
  // Replace NaN in the final product using the mask
  sum.data() = _mm_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 128>(fixed_point::fp64_t::NaN).data(), mask_nan.data());
  ...
  bool is_infinity = any_equal_to(mask_pos_inf | mask_neg_inf, VectorRegister<fixed_point::fp64_t, 128>::MaskAllBits());
    ...
    VectorRegister<fixed_point::fp64_t, 256> const &b)
{
  ...
  auto mask_nan  = (mask_pos_inf_a & mask_neg_inf_b)
                 | (mask_neg_inf_a & mask_pos_inf_b)
                 | (mask_nan_a | mask_nan_b);
  mask_pos_inf.data() = _mm256_blendv_epi8(mask_pos_inf.data(), _mm256_setzero_si256(), mask_nan.data());
  mask_neg_inf.data() = _mm256_blendv_epi8(mask_neg_inf.data(), _mm256_setzero_si256(), mask_nan.data());

  // Replace +inf in the final product using the mask
  sum.data() = _mm256_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 256>::MaskPosInf().data(), mask_pos_inf.data());
  // Replace -inf in the final product using the mask
  sum.data() = _mm256_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 256>::MaskNegInf().data(), mask_neg_inf.data());
  // Replace NaN in the final product using the mask
  sum.data() = _mm256_blendv_epi8(sum.data(), VectorRegister<fixed_point::fp64_t, 256>(fixed_point::fp64_t::NaN).data(), mask_nan.data());

  // Remove NaN/Inf elements from Overflow/Underflow mask
  mask_overflow = _mm256_blendv_epi8(mask_overflow, _mm256_setzero_si256(), (mask_nan | mask_pos_inf).data());
  mask_underflow = _mm256_blendv_epi8(mask_underflow, _mm256_setzero_si256(), (mask_nan | mask_neg_inf).data());
  bool is_overflow = _mm256_movemask_epi8(mask_overflow | mask_underflow) != 0;
  bool is_infinity = any_equal_to(mask_pos_inf | mask_neg_inf, VectorRegister<fixed_point::fp64_t, 256>::MaskAllBits());
    ...
    VectorRegister<fixed_point::fp64_t, 128> const &b)
{
  ...
  std::cout << "max128 = " << std::hex << static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MAX) << std::endl;
  std::cout << "min128 = " << std::hex << static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MIN) << std::endl;
  mask_overflow128[0] = prod128[0] > static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MAX);
  mask_overflow128[1] = prod128[1] > static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MAX);
  mask_underflow128[0] = prod128[0] < static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MIN);
  mask_underflow128[1] = prod128[1] < static_cast<fixed_point::fp64_t::NextType>(fixed_point::fp64_t::MIN);
  std::cout << "mask_overflow128 = " << std::hex << mask_overflow128[0] << ", " << mask_overflow128[0] << std::endl;
  std::cout << "mask_underflow128 = " << std::hex << mask_underflow128[0] << ", " << mask_underflow128[0] << std::endl;
  ...
  std::cout << "mask_underflow: " << VectorRegister<int64_t, 128>(mask_underflow) << std::endl;
  // Replace +inf in the final product using the mask
  prod.data() = _mm_blendv_epi8(prod.data(), VectorRegister<fixed_point::fp64_t, 128>::MaskPosInf().data(), mask_pos_inf.data());
  std::cout << "prod = " << VectorRegister<fixed_point::fp64_t, 128>(prod.data()) << std::endl;
  // Replace -inf in the final product using the mask
  prod.data() = _mm_blendv_epi8(prod.data(), VectorRegister<fixed_point::fp64_t, 128>::MaskNegInf().data(), mask_neg_inf.data());
  std::cout << "prod = " << VectorRegister<fixed_point::fp64_t, 128>(prod.data()) << std::endl;
  // Replace NaN in the final product using the mask
  prod.data() = _mm_blendv_epi8(prod.data(), VectorRegister<fixed_point::fp64_t, 128>(fixed_point::fp64_t::NaN).data(), mask_nan.data());
  std::cout << "prod = " << VectorRegister<fixed_point::fp64_t, 128>(prod.data()) << std::endl;
...
inline fixed_point::fp64_t reduce(VectorRegister<fixed_point::fp64_t, 256> const &x)
{
  ...
  bool is_nan = any_equal_to(VectorRegister<fixed_point::fp64_t, 256>::MaskNaN(x), VectorRegister<fixed_point::fp64_t, 256>::MaskAllBits());
  ...
  {    
    ...
    {
      if (fixed_point::fp64_t::CheckOverflow(static_cast<fixed_point::fp64_t::NextType>(sum.Data()) + static_cast<fixed_point::fp64_t::NextType>(x_[i].Data())))
      {
        fixed_point::fp64_t::fp_state |= fixed_point::fp64_t::STATE_OVERFLOW;
        return fixed_point::fp64_t::FP_MAX;
      }
      else if (fixed_point::fp64_t::CheckUnderflow(static_cast<fixed_point::fp64_t::NextType>(sum.Data()) + static_cast<fixed_point::fp64_t::NextType>(x_[i].Data())))
      {
        fixed_point::fp64_t::fp_state |= fixed_point::fp64_t::STATE_OVERFLOW;
