...
inline VectorRegister<float, 256> abs(VectorRegister<float, 256> const &a)
{
  ...
  auto const   ret  = VectorRegister<float, 256>(_mm256_andnot_ps(sign, a.data()));
  return ret;
}
...
inline VectorRegister<float, 128> abs(VectorRegister<float, 128> const &a)
{
  ...
  auto const   ret  = VectorRegister<float, 128>(_mm_andnot_ps(sign, a.data()));
  return ret;
}
...
inline VectorRegister<double, 256> abs(VectorRegister<double, 256> const &a)
{
  ...
  auto const    ret  = VectorRegister<double, 256>(_mm256_and_pd(mask, a.data()));
  return ret;
}
...
inline VectorRegister<double, 128> abs(VectorRegister<double, 128> const &a)
{
  ...
  auto const    ret  = VectorRegister<double, 128>(_mm_and_pd(mask, a.data()));
  return ret;
}
