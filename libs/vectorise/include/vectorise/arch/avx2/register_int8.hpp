...
template <>                                                                                    \
  ...
  {                                                                                            \
    ...
    __m128i hi1 = _mm_alignr_epi8(lo, hi, L * sizeof(type));                                   \
    __m128i lo1 = _mm_alignr_epi8(hi, lo, L * sizeof(type));                                   \
...
#undef FETCH_ROTATE_ELEMENTS_LEFT
...
inline bool all_equal_to(VectorRegister<int8_t, 128> const &x,
                         VectorRegister<int8_t, 128> const &y)
{
  __m128i r = (x == y).data();
  return _mm_movemask_epi8(r) == 0xFFFF;
...
inline bool all_equal_to(VectorRegister<int8_t, 256> const &x,
                         VectorRegister<int8_t, 256> const &y)
{
  __m256i r    = (x == y).data();
  auto    mask = static_cast<uint32_t>(_mm256_movemask_epi8(r));
...
inline bool any_equal_to(VectorRegister<int8_t, 128> const &x,
                         VectorRegister<int8_t, 128> const &y)
{
  __m128i r = (x == y).data();
  return _mm_movemask_epi8(r) != 0;
...
inline bool any_equal_to(VectorRegister<int8_t, 256> const &x,
                         VectorRegister<int8_t, 256> const &y)
{
  __m256i r = (x == y).data();
  return _mm256_movemask_epi8(r) != 0;
