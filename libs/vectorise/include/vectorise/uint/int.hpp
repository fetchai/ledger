...
public:
  ...
  enum
  {
    ...
    RESIDUAL_BITS     = WIDE_ELEMENTS * WIDE_ELEMENT_SIZE - INT_SIZE,
  };
  ...
  constexpr Int &operator<<=(std::size_t bits);
  constexpr Int &operator>>=(std::size_t bits);
...
constexpr Int<256> &Int<256>::operator*=(Int<256> const &n)
{
  ...
  __int128_t products[WIDE_ELEMENTS][WIDE_ELEMENTS] = {};
  ...
  for (std::size_t i = 0; i < WIDE_ELEMENTS; ++i)
  {
    for (std::size_t j = 0; j < WIDE_ELEMENTS; ++j)
    {
      ...
      products[i][j] =
          static_cast<__int128_t>(wide_[i]) * static_cast<__int128_t>(n.ElementAt(j));
    }
  }
  ...
  __int128_t carry = 0, terms[WIDE_ELEMENTS] = {};
...
template <uint16_t S>
constexpr Int<S> &Int<S>::operator<<=(std::size_t bits)
{
