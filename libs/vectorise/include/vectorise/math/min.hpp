...
template <typename T>
fetch::math::meta::IfIsNonFixedPointArithmetic<T, T> Min(T const &a, T const &b)
{
  return T(std::min(a, b));
}

template <typename T>
fetch::math::meta::IfIsFixedPoint<T, T> Min(T const &a, T const &b)
{
  return T::FromBase(std::min(a.Data(), b.Data()));
}

template <typename T>
VectorRegister<T, 8 * sizeof(T)> Min(VectorRegister<T, 8 * sizeof(T)> const &a,
...
T Min(VectorRegister<T, N> const &a)
{
