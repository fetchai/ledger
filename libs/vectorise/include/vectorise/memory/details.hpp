...
struct MatrixApplyFreeFunction {
  ...
  struct Unroll {
    typedef typename MatrixApplyFreeFunction< B,R, Args..., B const& >::template Unroll< Remaining...>::signature_type signature_type;

    static R Apply(B const* regs, signature_type &&fnc,  B& ret, Args &&...args ) {
      return MatrixApplyFreeFunction< B,R, Args..., B const& >
        ::template Unroll< Remaining...>::Apply(regs+1,std::move(fnc), ret, std::forward<Args>(args)..., *regs ) ;
    }
  };
...
struct MatrixReduceFreeFunction {
  ...
  template< typename T, typename ...Remaining > 
  struct Unroll {
    typedef typename MatrixReduceFreeFunction<B,   Args..., B const& >::template Unroll< Remaining...>::signature_type signature_type;

    static B Apply(B const &self, B const* regs, signature_type &&fnc,  Args &&...args ) {

      return MatrixReduceFreeFunction<B, Args..., B const& >
        ::template Unroll< Remaining...>::Apply(self, regs+1,std::move(fnc), std::forward<Args>(args)..., *regs ) ;
    }
  };
...
struct MatrixApplyClassMember {
  ...
  template< typename T, typename ...Remaining > 
  struct Unroll {
    typedef typename MatrixApplyClassMember< C, B,R, Args..., B const& >::template Unroll< Remaining...>::signature_type signature_type;          
    static R Apply(B const* regs, C const &cls, signature_type &&fnc,  B& ret, Args ...args ) {
      return MatrixApplyClassMember< C, B,R, Args..., B const& >
        ::template Unroll< Remaining...>::Apply(regs+1, cls, std::move(fnc), ret, args..., *regs ) ;
    }

    static R Apply(B const **regs, C const &cls,
      signature_type &&fnc, B& ret, Args ...args) {
      return MatrixApplyClassMember< C, B,R, Args..., B const& >
        ::template Unroll< Remaining...>::Apply(regs+1, cls, std::move(fnc), ret, args..., **regs );
