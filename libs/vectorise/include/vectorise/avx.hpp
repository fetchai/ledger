...
#define FETCH_ADD_OPERATOR(op, type, fnc)                                       \
  inline VectorRegister<type, 256> operator op(                                 \
      VectorRegister<type, 256> const &a,                                       \
...
#define FETCH_ADD_OPERATOR(op, type, fnc)                                                                 \
  inline VectorRegister<type, 256> operator op(                                                           \
    ...
    type done[VectorRegister<type, 256>::E_BLOCK_COUNT] = {1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f};      \
...
inline VectorRegister<double, 256> vector_zero_below_element( VectorRegister<double, 256> const &a, int const &n) {
...
inline VectorRegister<double, 256> vector_zero_above_element( VectorRegister<double, 256> const &a, int const &n) {
  alignas(16) uint64_t mask[2]= { uint64_t( -(0 <= n) ), uint64_t( -(1 <= n) ) };   
...
inline VectorRegister<float, 256> vector_zero_below_element( VectorRegister<float, 256> const &a, int const &n) {
...
inline VectorRegister<float, 256> vector_zero_above_element( VectorRegister<float, 256> const &a, int const &n) {
  alignas(16) const uint32_t mask[4]= { uint32_t( -( 0 <= n) ),uint32_t( -(1 <= n) ),uint32_t( -(2 <= n) ), uint32_t( -(3 <= n ) ) };
...
inline VectorRegister<float, 256> shift_elements_left(VectorRegister<float, 256> const &x) {
  ...
  return VectorRegister<float, 256> (  _mm256_blend_ps(t0, t1, 0x88) );         // [ 0  x7  x6  x5  x4  x3  x2  x1]
