...
int main(int argc, char **argv) {
  ...
  if((argc>=2) &&  (std::string(argv[1]) == "comp") ) {
    ...
    for(std::size_t i=0; i < M; ++i) {
      ...
      a.in_parallel().Apply([](vector_register_type const &x, vector_register_type &y) {

          static vector_register_type one(1);

          // We approximate the exponential function by a clever first order Taylor expansion
